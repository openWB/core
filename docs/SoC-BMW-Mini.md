...
### openWB 2.x: Mehrere Fahrzeuge in einem Account
Wenn mehrere Fahrzeuge in einem Connected-Drive-Account vorhanden sind, ist dies in openWB 2.x wie folgt zu konfigurieren:

Das erste Fahrzeug - mit der niedrigsten ID - wird wie wie oben beschrieben konfiguriert.

Ab dem 2. Fahrzeug wird als Captcha-Token der Wert SECONDARY (genau so in Grossbuchstaben) eingegeben.
