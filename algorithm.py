                            ...
                            if chargepoint.data["set"]["charging_ev"] != -1:
                                ...
                                if ((charging_ev.charge_template.data["prio"] == prio and 
                                    ...
                                    if ((chargepoint.data["set"]["current"] - charging_ev.ev_template.data["nominal_difference"]) < max(chargepoint.data["get"]["current"]) and
                                        ...
                                        if (-2-charging_ev.ev_template.data["nominal_difference"]) < (new_current - max(chargepoint.data["get"]["current"])) < (2+charging_ev.ev_template.data["nominal_difference"]):
                                            current = new_current
                                        else:
    ...
    try:
        ...
        if bat_overhang > 0:
            ...
            if return_power != 0:
                ...
                required_current = chargepoint.data["set"]["charging_ev"].check_min_max_current_for_pv_charging(required_current, phases)
