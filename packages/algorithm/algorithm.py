...
class control:
    ...
    def __init__(self):
        ...
        self.chargemodes = (("scheduled_charging", "instant_charging",
                             True), ("scheduled_charging", "instant_charging",
                                     False), (None, "time_charging", True),
                            (None, "time_charging",
                             False), ("instant_charging", "instant_charging",
                                      True), ("instant_charging",
                                              "instant_charging", False),
                            ("pv_charging", "instant_charging",
                             True), ("pv_charging", "instant_charging", False),
                            ("scheduled_charging", "pv_charging",
                             True), ("scheduled_charging", "pv_charging",
                                     False), ("pv_charging", "pv_charging",
                                              True), ("pv_charging",
                                                      "pv_charging", False),
                            (None, "standby", True), (None, "standby", False),
                            (None, "stop", True), (None, "stop", False))
    ...
    def calc_current(self):
        ...
        try:
            ...
            log.MainLogger().debug("EVU-Punkt: Leistung[W] " + str(
                data.data.counter_data[evu_counter].data["get"]["power_all"]) +
                                   ", Stroeme[A] " +
                                   str(data.data.counter_data[evu_counter].
                                       data["get"]["current"]))
            if data.data.counter_data["all"].data["set"][
                    "loadmanagement_available"] is False:
                return

            # erstmal die PV-Überschuss-Ladung zurück nehmen
            log.MainLogger().info(
                "## Ueberschuss-Ladung ueber Mindeststrom bei PV-Laden zuruecknehmen."
            )
                ...
                try:
                    if "cp" in cp:
                        ...
                        if chargepoint.data["set"]["charging_ev"] != -1:
                            if chargepoint.data["set"]["charging_ev_data"].data[
                                    "control_parameter"][
                                        "submode"] == "pv_charging" and chargepoint.data[
                                            "get"]["charge_state"]:
                                if data.data.pv_data[
                                        "all"].switch_off_check_timer(
                                            chargepoint):
                                    # Ladung stoppen
                                    required_current = 0
                                    ...
                                    chargepoint.data["set"][
                                        "charging_ev_data"].data[
                                            "control_parameter"][
                                                "submode"] = "stop"
                except Exception as e:
                    log.MainLogger().exception(
                        "Fehler im Algorithmus-Modul fuer Ladepunkt" + cp)
        except Exception as e:
                            ...
                            if ((charging_ev.charge_template.data["chargemode"]
                                ...
                                if max(chargepoint.data["get"]
                                    ...
                                    if released_current < 0:
                                        data.data.pv_data[
                                            "all"].allocate_evu_power(
                                                released_current * 230 *
                                                chargepoint.data["get"]
                                                ["phases_in_use"])
                                        self._process_data(
                                            chargepoint, charging_ev.
                                            data["control_parameter"]
                                            ["required_current"])
                                        log.MainLogger().debug(
                                            "Ladung an LP" +
                                            str(chargepoint.cp_num) + " um " +
                                            str(released_current) + "A auf " +
                                            str(charging_ev.
                                                data["control_parameter"]
                                                ["required_current"]) +
                                            "A angepasst.")
        ...
        try:
            ...
            if loadmanagement_state:
                log.MainLogger().warning(
                    "## Ladung wegen aktiven Lastmanagements stoppen.")
                # Zähler mit der größten Überlastung ermitteln
                overloaded_counters = sorted(overloaded_counters.items(),
                                             key=lambda e: e[1][1],
                                             reverse=True)
                ...
                # EVs, die nicht laden, sollen nicht berücksichtigt werden.
                for cp in data.data.cp_data:
                    try:
                        if "cp" in cp:
                            if data.data.cp_data[cp].data["set"][
                                    "current"] > max(
                                        data.data.cp_data[cp].data["get"]
                                        ["current"]
                ...
                for b in range(0, len(data.data.counter_data) * 2):
                    chargepoints = data.data.counter_data[
                        "all"].get_chargepoints_of_counter(
                    ...
                    for mode in reversed(self.chargemodes[:-4]):
                        overshoot = self._down_regulation(
                            mode, chargepoints, overshoot,
                            overloaded_counters[n][1][1])
                        if overshoot == 0:
                            break
                    ...
                    for cp in data.data.cp_data:
                        try:
                            ...
                            log.MainLogger().exception(
                                "Fehler im Algorithmus-Modul fuer Ladepunkt" +
                                cp)
                    else:
                        log.MainLogger().warning(
                            "Ueberlastung wird durch Hausverbrauch verursacht."
                        )
                        break
                    if overshoot == 0:
                    ...
                    if loadmanagement_state is False:
    ...
    def _down_regulation(self,
                    ...
                    if "current" in chargepoint.data["set"]:
                        ...
                        if chargepoint.data["set"]["charging_ev"] != -1:
                            ...
                            if chargepoint.data["get"]["charge_state"] is False:
                                phases = charging_ev.data["control_parameter"][
                                    "phases"]
                            else:
                                phases = chargepoint.data["get"][
                                    "phases_in_use"]
                            if ((charging_ev.charge_template.data["prio"]
                                 == prio) and
                                (charging_ev.charge_template.data["chargemode"]
                                 ["selected"] == mode or mode is None)
                                    and (charging_ev.data["control_parameter"]
                                         ["submode"] == submode)
                                    and (phases >= max_overshoot_phase)):
        ...
        if len(preferenced_chargepoints) != 0:
            log.MainLogger().debug("Zu reduzierende Ladepunkte " +
                                   str(preferenced_chargepoints) +
                                   " in Lademodus " + str(mode) +
                                   " Submodus " + str(submode) + " Prio " +
                                   str(prio))
        return self._perform_down_regulation(preferenced_chargepoints,
                                             max_current_overshoot,
                                             max_overshoot_phase, prevent_stop)

    def _perform_down_regulation(self,
                                 preferenced_chargepoints,
                                 max_current_overshoot,
                                 max_overshoot_phase,
                                 prevent_stop=False):
        ...
        try:
            if len(preferenced_chargepoints) == 0:
                ...
                for cp in preferenced_chargepoints:
                    try:
                        # Wenn der LP erst in diesem Zyklus eingeschaltet wird, sind noch keine phases_in_use hinterlegt.
                        if cp.data["get"]["charge_state"] is False:
                        ...
                        if cp.data["set"]["current"] != 0:
                            ...
                            considered_current = cp.data["set"][
                        ...
                        if adaptable_current <= 0:
                            ...
                            if prevent_stop is False:
                                ...
                                cp.data["set"]["charging_ev_data"].data[
                                    "control_parameter"][
                                        "required_current"] = 0
                        ...
                        data.data.counter_data[data.data.counter_data[
                            "all"].get_evu_counter()].print_stats()
                        # Wenn max_overshoot_phase -1 ist, wurde die maximale Gesamtleistung überschrittten und max_current_overshoot muss,
    ...
    def _adjust_chargepoints(self, mode_tuple):
        ...
        try:
                    ...
                    if "cp" in cp:
                        ...
                        if "current" in chargepoint.data["set"]:
                            ...
                            if chargepoint.data["set"]["charging_ev"] != -1:
                                ...
                                if ((charging_ev.charge_template.data["prio"]
                                     == prio) and
                                    (charging_ev.charge_template.
                                     data["chargemode"]["selected"] == mode
                                     or mode is None) and
                                    (charging_ev.data["control_parameter"]
                                     ["submode"] == submode) and
                                    (chargepoint.data["set"]
            ...
            if len(preferenced_chargepoints) != 0:
                ...
                log.MainLogger().debug("Hochzuregelnde Ladepunkte " +
                                       str(preferenced_chargepoints) +
                                       " in Lademodus " + str(mode) +
                                       " Submodus " + str(submode) + " Prio " +
                                       str(prio))
                for cp in preferenced_chargepoints:
                    try:
                        ...
                        log.MainLogger().debug("Lastmanagement aktiv: " +
                                               str(loadmanagement_state))
                        if loadmanagement_state:
                            overloaded_counters = sorted(
                                overloaded_counters.items(),
                                key=lambda e: e[1][1],
                                reverse=True)
                            # Wenn max_overshoot_phase -1 ist, wurde die maximale Gesamtleistung überschrittten und max_current_overshoot muss,
                            # wenn weniger als 3 Phasen genutzt werden, entsprechend multipliziert werden.
                            if overloaded_counters[0][1][
                                    1] == -1 and phases < 3:
                                undo_missing_current = (
                                    overloaded_counters[0][1][0] *
                                    (3 - phases + 1)) * -1
                            else:
                                undo_missing_current = overloaded_counters[0][
                                    1][0] * -1
                            ...
                            if undo_missing_current * -1 > missing_current:
                                ...
                                log.MainLogger().debug(
                                    "Keine Hochregelung fuer Ladepunkt " +
                                    str(cp) +
                            ...
                            # Es kann nur ein Teil des fehlenden Ladestroms hochgeregelt werden.
                                ...
                                # Werte aktualisieren
                                loadmanagement.loadmanagement_for_cp(
                                    cp, required_power, undo_missing_current,
                                    phases)
                                self._process_data(
                                    cp, cp.data["set"]["current"] +
                                    missing_current + undo_missing_current)
                                message = "Das Lastmanagement hat den Ladestrom um " + \
                                    str(round(
                        ...
                        # Zuvor fehlender Ladestrom kann nun genutzt werden
                            ...
                            self._process_data(
                                cp,
                                cp.data["set"]["current"] + missing_current)
                            message = "Das Lastmanagement hat den Ladestrom um " + \
                        ...
                        log.MainLogger().exception(
                            "Fehler im Algorithmus-Modul fuer Ladepunkt" + cp)
                data.data.counter_data[data.data.counter_data["all"].
                                       get_evu_counter()].print_stats()
        except Exception as e:
            log.MainLogger().exception("Fehler im Algorithmus-Modul")
    ...
    def _switch_off_threshold(self, mode_tuple):
            ...
            try:
                if "cp" in cp:
                    ...
                    if chargepoint.data["set"]["charging_ev"] != -1:
                        charging_ev = chargepoint.data["set"][
                            "charging_ev_data"]
                        if ((charging_ev.charge_template.data["prio"] == prio)
                                and
                            (charging_ev.charge_template.data["chargemode"]
                             ["selected"] == mode or mode is None) and
                            (charging_ev.data["control_parameter"]["submode"]
                             == submode)):
                            valid_chargepoints[chargepoint] = None
            except Exception as e:
                log.MainLogger().exception(
        ...
        if len(preferenced_chargepoints) == 0:
            ...
            log.MainLogger().debug("Switch-Off-Threshold fuer Ladepunkte " +
                                   str(preferenced_chargepoints) +
                                   " in Lademodus " + str(mode) +
                                   " Submodus " + str(submode) + " Prio " +
                                   str(prio) + " pruefen.")
            ...
            try:
                if "cp" in cp:
                    if data.data.cp_data[cp].data["set"]["charging_ev"] != -1:
                        chargepoint = data.data.cp_data[cp]
                        charging_ev = data.data.cp_data[cp].data["set"][
                            "charging_ev_data"]
                        if (chargepoint.data["config"]["auto_phase_switch_hw"]
                                and chargepoint.data["get"]["charge_state"]
                                and charging_ev.data["control_parameter"]
                            ["chargemode"] == "pv_charging"
                                and data.data.general_data["general"].
                                get_phases_chargemode("pv_charging") == 0
                                and charging_ev.data["control_parameter"]
                            ...
                            log.MainLogger().debug(
                                "Ladepunkt " + str(cp) +
                                ": Pruefen, ob Phasenumschaltung durchgefuehrt werden soll."
                            )
                            phases, current, message = charging_ev.auto_phase_switch(
                                ...
                                ["required_current"],
                                charging_ev.data["control_parameter"]
                                ["phases"], chargepoint.data["get"]["current"])
                            if message is not None:
                            ...
                            required_current = charging_ev.check_min_max_current(
                                current, charging_ev.data["control_parameter"]
                                ["phases"])
                            charging_ev.data["control_parameter"][
    ...
    def _manage_distribution(self):
        ...
        """
        try:
            log.MainLogger().info("## Zuteilung des Ueberschusses")
            for mode_tuple in self.chargemodes:
                try:
                    ...
                    if len(preferenced_chargepoints) != 0:
                        log.MainLogger().debug("Zuteilung fuer Ladepunkte " +
                                               str(preferenced_chargepoints) +
                                               " in Lademodus " + str(mode) +
                                               " Submodus " + str(submode) +
                                               " Prio " + str(prio))
                        current_mode = self.chargemodes.index(mode_tuple)
                        self._distribute_power_to_cp(preferenced_chargepoints,
                                                     current_mode)
                except Exception as e:
                    log.MainLogger().exception(
                        "Fehler im Algorithmus-Modul fuer Ladepunkt" + cp)
                ...
                log.MainLogger().info(
                    "## Zuteilung beendet, da kein Ladepunkt mehr auf Zuteilung wartet."
                )
        except Exception as e:
            log.MainLogger().exception("Fehler im Algorithmus-Modul")
    ...
    def _distribute_power_to_cp(self, preferenced_chargepoints, current_mode):
        ...
        for chargepoint in preferenced_chargepoints:
            try:
                ...
                required_current = charging_ev.data["control_parameter"][
                    "required_current"]
                required_power = phases * 230 * \
                    charging_ev.data["control_parameter"]["required_current"]
                pub.pub(
                    "openWB/set/chargepoint/" + str(chargepoint.cp_num) +
                ...
                if charging_ev.data["control_parameter"][
                    ...
                    self._calc_pv_charging(chargepoint, required_power,
                                           ...
                                           current_mode)
                elif (charging_ev.data["control_parameter"]["submode"]
                      == "stop"
                      or (charging_ev.data["control_parameter"]["submode"]
                          == "standby")):
                    required_current = 0
                ...
                log.MainLogger().exception(
                    "Fehler im Algorithmus-Modul fuer Ladepunkt" + chargepoint)

    def _calc_normal_load(self, chargepoint, required_power, required_current,
                          phases, current_mode):
        ...
        try:
            ...
            log.MainLogger().debug(
                "Benoetigte neue/zusaetzliche Ladeleistung " +
                str(power_to_allocate) +
                "W, benoetigter neuer/zusaetzlicher Strom " +
                str(current_to_allocate) + "A")
            _, overloaded_counters = allocate_power(chargepoint,
                                                    power_to_allocate,
                                                    current_to_allocate,
                                                    phases)
            self._process_data(chargepoint, required_current)
            ...
            if data.data.counter_data["all"].data["set"][
                    "loadmanagement_active"] and len(overloaded_counters) != 0:
                # Lastmanagement hat eingegriffen
                log.MainLogger().debug(
                    "Aktuell kalkulierte Stroeme am EVU-Punkt[A]: " +
                    str(data.data.counter_data[evu_counter].data["set"]
                        ["current_used"]))
                log.MainLogger().warning(
                ...
                for mode in reversed(self.chargemodes[(current_mode + 1):-4]):
                    try:
                        ...
                        if remaining_current_overshoot <= 0:
                            ...
                            self._process_data(
                                ...
                                data["control_parameter"]["required_current"])
                            break
                        else:
                            # Abschalten
                            if chargepoint.data["set"][
                                ...
                                if remaining_current_overshoot <= 0:
                                    # LP kann nun wie gewünscht eingeschaltet werden
                                    self._process_data(
                                        chargepoint, chargepoint.data["set"]
                                        ["charging_ev_data"].
                                        data["control_parameter"]
                                        ["required_current"])
                                    break
                        ...
                        log.MainLogger().exception(
                            "Fehler im Algorithmus-Modul fuer Modus " +
                            str(mode))
                else:
                    ...
                    if remaining_current_overshoot != 0:
                        ...
                        log.MainLogger().info("LP " + str(chargepoint.cp_num) +
                                              ": " + message)
                        log.MainLogger().debug(
                            "Wiederherstellen des Zustands, bevor LP" +
                            str(chargepoint.cp_num) + " betrachtet wurde.")
                        chargepoint.data["get"]["state_str"] = message
                        self._process_data(chargepoint, 0)
            else:
                (log.MainLogger().info(
                    "LP: " + str(chargepoint.cp_num) + ", Ladestrom: " +
                    str(chargepoint.data["set"]["current"]) + "A, Phasen: " +
                    str(chargepoint.data["set"]["charging_ev_data"].
                        data["control_parameter"]["phases"]) +
                    ", Ladeleistung: " +
                    str((chargepoint.data["set"]["charging_ev_data"].
                         data["control_parameter"]["phases"] *
    ...
    def _calc_pv_charging(self, chargepoint, required_power, required_current,
                          phases, current_mode):
        ...
        try:
            if (chargepoint.data["set"]["charging_ev_data"].charge_template.
                    data["chargemode"]["selected"] == "scheduled_charging"
                    and "pv_charging" not in chargepoint.data["set"]
                ...
                if self._check_cp_without_feed_in_is_prioritised(chargepoint):
                    set_current, threshold_not_reached = data.data.pv_data[
                        "all"].switch_on(
                            chargepoint, required_power, required_current,
                            phases,
            ...
            if threshold_not_reached:
                ...
                for mode_tuple in reversed(self.chargemodes[current_mode +
                                                            1:-4]):
                    mode = mode_tuple[0]
                    submode = mode_tuple[1]
                    prio = mode_tuple[2]
                        ...
                        try:
                            if "cp" in item:
                                ...
                                if cp.data["set"]["charging_ev"] != -1:
                                    charging_ev = cp.data["set"][
                                        "charging_ev_data"]
                                    # set-> current enthält einen Wert, wenn das EV in diesem Zyklus eingeschaltet werden soll, aktuell aber noch nicht lädt.
                                    if "current" in cp.data["set"]:
                                        if cp.data["set"]["current"] == 0:
                                            continue
                                    if ((charging_ev.charge_template.
                                         data["prio"] == prio) and
                                        (charging_ev.charge_template.
                                         data["chargemode"]["selected"] == mode
                                         or mode is None) and
                                ...
                                if cp.data["set"][
                                    ...
                                    data.data.pv_data[
                                        "all"].allocate_evu_power(
                                            ...
                                            data["control_parameter"]
                                            ["required_current"] * 230 *
                                            chargepoint.data["get"]
                                            ["phases_in_use"])
                                    self._process_data(cp, 0)
                                    log.MainLogger().warning(
                                        "LP" + str(cp.cp_num) +
                                        " abgeschaltet, um die Einschaltverzoegerung an LP"
                                        + str(chargepoint.cp_num) +
                                        " zu starten.")
                                    # switch_on erneut durchführen
                                    set_current, threshold_not_reached = data.data.pv_data[
                                        "all"].switch_on(
                                            chargepoint, required_power,
                                            required_current, phases,
                                            data.data.bat_data["all"].
                                            power_for_bat_charging())
                                    if threshold_not_reached is False:
                                        break
                            except Exception as e:
                                log.MainLogger().exception(
                                    "Fehler im Algorithmus-Modul fuer Ladepunkt"
                    ...
                    data.data.cp_data = cp_data_old
                    log.MainLogger().info(
                        "Keine Ladung an LP" + str(chargepoint.cp_num) +
    ...
    def _distribute_unused_evu_overhang(self):
        ...
        """
        try:
            log.MainLogger().info("## Uebrigen Ueberschuss verteilen.")
            for mode in self.chargemodes[6:-4]:
                overhang = self._get_bat_and_evu_overhang()
                if overhang != 0:
                    if overhang > 0.01:
                        self._distribute_remaining_overhang(mode, False)
                    if (overhang - data.data.general_data["general"].
                            data["chargemode_config"]["pv_charging"]
                        ["feed_in_yield"]) > 0.01:
    ...
    def _distribute_remaining_overhang(self, mode_tuple, feed_in_limit):
        ...
        try:
            ...
            for chargepoint in data.data.cp_data:
                try:
                    if "cp" in chargepoint:
                        if data.data.cp_data[chargepoint].data["set"][
                                "charging_ev"] != -1:
                            charging_ev = data.data.cp_data[chargepoint].data[
                                "set"]["charging_ev_data"]
                            if ((
                                (charging_ev.charge_template.data["prio"]
                                 == prio) and
                                (charging_ev.charge_template.data["chargemode"]
                                 ["selected"] == mode or mode is None) and
                                (charging_ev.data["control_parameter"]
                                 ["submode"] == submode))
                                    and data.data.cp_data[chargepoint].
                                    data["set"]["current"] != 0):
                                # Erst hochregeln, wenn geladen wird.
                                if ((data.data.cp_data[chargepoint].data["set"]
                                     ["current"] - charging_ev.ev_template.
                                     data["nominal_difference"]) < max(
                                         data.data.cp_data[chargepoint].
                                         data["get"]["current"])
                                        and charging_ev.charge_template.
                                        data["chargemode"]["pv_charging"]
            ...
            if num_of_ev > 0:
                ...
                bat_overhang = data.data.bat_data[
                    "all"].power_for_bat_charging()
                if feed_in_limit is False:
                    diff_per_ev_power = (
                        data.data.pv_data["all"].overhang_left() +
                        bat_overhang) / num_of_ev
                else:
                    feed_in_yield = data.data.general_data["general"].data[
                        "chargemode_config"]["pv_charging"]["feed_in_yield"]
                    if data.data.pv_data["all"].overhang_left(
                    ) <= feed_in_yield:
                        diff_per_ev_power = (
                            data.data.pv_data["all"].overhang_left() -
                            feed_in_yield + bat_overhang) / num_of_ev
                        log.MainLogger().debug("diff_per_ev_power " +
                                               str(diff_per_ev_power))
                    else:
                        ...
                        log.MainLogger().debug("dif_per_ev_current " +
                                               str(dif_per_ev_current))
                            ...
                            if "cp" in cp:
                                ...
                                if chargepoint.data["set"]["charging_ev"] != -1:
                                    ...
                                    if ((charging_ev.charge_template.
                                         data["prio"] == prio and
                                         (charging_ev.charge_template.
                                          data["chargemode"]["selected"]
                                          == mode or mode is None) and
                                         charging_ev.data["control_parameter"]
                                         ["submode"] == submode) and
                                            chargepoint.data["set"]["current"]
                                            != 0):
                                        if ((chargepoint.data["set"]["current"]
                                            ...
                                            ["pv_charging"]["feed_in_limit"]
                                                == feed_in_limit):
                                            if chargepoint.data["get"][
                                                    "charge_state"]:
                                                phases = chargepoint.data[
                                                    "get"]["phases_in_use"]
                                            else:
                                                phases = chargepoint.data["set"][
                                                    "charging_ev_data"].data[
                                                        "control_parameter"][
                                                            "phases"]
                                            if diff_per_ev_power != 0:
                                                new_current = (
                                                    diff_per_ev_power / 230 /
                                                    phases) + chargepoint.data[
                                                        "set"]["current"]
                                            else:
                                            ...
                                            if (
                                                    ...
                                                    new_current -
                                                    max(chargepoint.data["get"]
                                                        ["current"])
                                            ) < (5 + charging_ev.ev_template.
                                                 data["nominal_difference"]):
                                                current = new_current
                                                log.MainLogger().debug(
                                                    "current 1 " +
                                            ...
                                            chargepoint.data["set"][
                                                "current"] = current
                                            log.MainLogger().info(
                                                "Überschussladen an LP: " +
                                                str(chargepoint.cp_num) +
                                                ", Ladestrom: " +
                                                str(current) + "A, Phasen: " +
                                                str(phases) +
                                                ", Ladeleistung: " +
                                                str(phases * 230 * current) +
                                                "W")
                        except Exception as e:
                            log.MainLogger().exception(
                                "Fehler im Algorithmus-Modul fuer Ladepunkt" +
        ...
        try:
            ...
            if valid_chargepoints:
                while len(valid_chargepoints) > 0:
                    ...
                    extreme_cp = [
                        key for key in valid_chargepoints
                        if valid_chargepoints[key] == extreme_value
                    ]
                    if len(extreme_cp) > 1:
                ...
                for cp in data.data.cp_data:
                    try:
                        if "cp" in cp:
                            if data.data.cp_data[cp].data["set"][
                                    "charging_ev"] != -1:
                                charging_ev = data.data.cp_data[cp].data[
                                    "set"]["charging_ev_data"]
                                if (charging_ev.data["control_parameter"]
                                    ["submode"] == "pv_charging"
                                        and charging_ev.charge_template.
                                    ...
                                    if data.data.cp_data[cp].data["set"][
                                            "charging_ev_data"].data[
                                                "control_parameter"][
                                                    "phases"] == 1:
                                        max_current = charging_ev.ev_template.data[
    ...
    def _get_bat_and_evu_overhang(self):
        ...
        """
        try:
            return data.data.bat_data["all"].power_for_bat_charging(
            ) + data.data.pv_data["all"].overhang_left()
        except Exception as e:
            log.MainLogger().exception("Fehler im Algorithmus-Modul")
            return 0
...
def use_evu_bat_power(chargepoint,
                      required_power,
                      required_current,
                      phases,
                      pv_mode=True):
    ...
    try:
        ...
        if return_power != 0:
            ...
            required_current = chargepoint.data["set"][
                "charging_ev_data"].check_min_max_current(required_current,
                                                          phases,
                                                          pv=True)
        loadmanagement_state, overloaded_counters = loadmanagement.loadmanagement_for_cp(
            chargepoint, required_power - return_power,
            (required_power - return_power) / 230 / phases, phases)
        if loadmanagement_state:
            overloaded_counters = sorted(overloaded_counters.items(),
                                         key=lambda e: e[1][1],
                                         reverse=True)
            ...
            new_current = chargepoint.data["set"][
                "charging_ev_data"].check_min_max_current(required_current,
                                                          phases,
                                                          pv=True)
            ...
            loadmanagement.loadmanagement_for_cp(chargepoint, missing_power,
                                                 missing_current, phases)
    except Exception as e:
        log.MainLogger().exception("Fehler im Algorithmus-Modul")
