            ...
            for cp in data.data.cp_data:
                try:
                    ...
                    log.MainLogger().exception(
                        "Fehler in der allgemeinen Ladepunkt-Klasse fuer Ladepunkt "
                        + cp)
            else:
                data.data.pv_data["all"].reset_pv_data()
        except Exception as e:
            log.MainLogger().exception(
                "Fehler in der allgemeinen Ladepunkt-Klasse")
                    ...
                    try:
                        ...
                        if chargepoint.template.data["rfid_enabling"]:
                            ...
                            if chargepoint.data["config"]["connection_module"][
                                ...
                                if chargepoint.data["config"][
                                        "connection_module"]["config"][
                                            "external_openwb"][
                                                "chargepoint"] == 2:
                ...
                for cp in chargepoints:
                    try:
                        if "cp" in cp:
                            chargepoint = data.data.cp_data[cp]
                            if chargepoint.template.data["rfid_enabling"]:
                                if chargepoint.data["get"]["read_tag"][
                                        "tag"] != "0":
                                    # Darf mit diesem Tag der LP freigeschaltet werden?
                                    if chargepoint.data["get"]["read_tag"][
                                            "tag"] in chargepoint.template.data[
                                                "valid_tags"] or len(
                                                    chargepoint.template.
                                                    data["valid_tags"]) == 0:
                                        chargepoint.data["set"][
                                            "rfid"] = chargepoint.data["get"][
                                                "read_tag"]["tag"]
                                        pub.pub(
                                            "openWB/set/chargepoint/" +
                                            str(chargepoint.cp_num) +
                                            "/set/rfid",
                                            chargepoint.data["set"]["rfid"])
                                        log.MainLogger().info(
                                            "LP " + str(chargepoint.cp_num) +
                                            " wurde Tag " +
                                        ...
                                        chargepoint.data["get"][
                                            "state_str"] = "Tag " + str(
                                                chargepoint.data["get"]
                                                ["read_tag"]["tag"]
                                            ) + " ist an diesem Ladepunkt nicht gueltig."
                                        log.MainLogger().info(
                                            "LP " + str(chargepoint.cp_num) +
                                            ": Tag " +
                                            str(chargepoint.data["get"]
                                                ["read_tag"]["tag"]) +
                                            " ist an diesem Ladepunkt nicht gueltig."
                                        )

                                    # Verarbeiteten Tag löschen
                                    chargepoint.data["get"]["read_tag"][
                                        "tag"] = "0"
                                    chargepoint.data["get"]["read_tag"][
                                        "timestamp"] = "0"
                                    pub.pub(
        ...
        try:
            ...
            for cp in chargepoints:
                try:
                    if "cp" in cp:
                        ...
                        if chargepoint_read.data["get"]["read_tag"][
                                "tag"] != "0":
                            read_tag = chargepoint_read.data["get"][
                                "read_tag"]["tag"]
                            # Scannen darf nicht länger als 5 Min zurück liegen
                            if timecheck.check_timestamp(
                                    chargepoint_read.data["get"]["read_tag"]
                                ["timestamp"], 300) is False:
                                # abgelaufen
                                chargepoint_read.data["get"]["read_tag"][
                                    "tag"] = "0"
                                chargepoint_read.data["get"]["read_tag"][
            ...
            if read_tag is not None:
                ...
                for cp in chargepoints:
                    try:
                        if "cp" in cp:
                            ...
                            if chargepoint_match.template.data[
                                    "rfid_enabling"]:
                                ...
                                if chargepoint_match.data["set"][
                                        "rfid"] == "0" and chargepoint_match.data[
                                            "get"]["plug_state"]:
                                    if plug_time == 0 or timecheck.get_difference(
                                            plug_time, chargepoint_match.
                                            data["set"]["plug_time"]) < 0:
                                        plug_time = chargepoint_match.data[
                                            "set"]["plug_time"]
                ...
                if last_plugged_cp is None:
                    log.MainLogger().info(
                        "Es wurde kein EV angesteckt, dessen Ladepunkt auf eine Zuweisung per RFID wartet. Gescannter Tag: "
                        + str(read_tag))
                else:
                    # Darf mit diesem Tag der LP freigeschaltet werden?
                    if read_tag in last_plugged_cp.template.data[
                            "valid_tags"] or len(last_plugged_cp.template.
                                                 data["valid_tags"]) == 0:
                        last_plugged_cp.data["set"]["rfid"] = read_tag
                        pub.pub(
                            "openWB/set/chargepoint/" +
                            str(last_plugged_cp.cp_num) + "/set/rfid",
                            last_plugged_cp.data["set"]["rfid"])
                        log.MainLogger().info(
                            "LP " + str(last_plugged_cp.cp_num) +
                            " wurde Tag " +
                            str(last_plugged_cp.data["set"]["rfid"]) +
                            " zugeordnet.")
                        ...
                        pub.pub(
                            "openWB/set/chargepoint/" +
                            str(chargepoint_read.cp_num) + "/get/read_tag",
                            chargepoint_read.data["get"]["read_tag"])
                    else:
                        log.MainLogger().info("Der Tag " + str(read_tag) +
                                              " ist an Ladepunkt " +
                                              str(last_plugged_cp.cp_num) +
                                              " nicht gueltig.")
                        chargepoint_read.data["get"]["read_tag"]["tag"] = "0"
                        chargepoint_read.data["get"]["read_tag"][
                            "timestamp"] = "0"
...
class chargepoint:
    ...
    def __init__(self, index):
        try:
            ...
            log.MainLogger().exception("Fehler in der Ladepunkt-Klasse von " +
                                       str(self.cp_num))
                ...
                if general_data["grid_protection_active"]:
                    if general_data["grid_protection_timestamp"] != "0":
                        ...
                        if timecheck.check_timestamp(
                            ...
                            pub.pub(
                                "openWB/set/general/grid_protection_timestamp",
                                "0")
                            pub.pub(
                                "openWB/set/general/grid_protection_random_stop",
                                0)
                    else:
                        state = False
                        message = "Ladepunkt gesperrt, da der Netzschutz aktiv ist."
    ...
    def _is_ripple_control_receiver_inactive(self):
        ...
        try:
            ...
            if general_data["ripple_control_receiver"]["configured"]:
                if general_data["ripple_control_receiver"][
                        "r1_active"] or general_data[
                            "ripple_control_receiver"]["r2_active"]:
                    state = False
    ...
    def _is_autolock_inactive(self):
        ...
        try:
            message = None
            state = self.template.autolock(self.data["set"]["autolock_state"],
            ...
            if state is False:
                ...
                if data.data.optional_data["optional"].data["rfid"][
                        "active"] and self.template.data["rfid_enabling"]:
                    if self.data["set"]["rfid"] == "0":
                        state = False
                        message = "Keine Ladung, da der Ladepunkt durch Autolock gesperrt ist. Eine Freischaltung per RFID ist möglich."
                    else:
                        state = True
                        message = None
                else:
        ...
        try:
            ...
            if state is False:
                ...
                if self.data["set"]["plug_time"] == "0":
                    ...
                    pub.pub(
                        "openWB/set/chargepoint/" + str(self.cp_num) +
    ...
    def get_state(self):
        ...
        try:
            ...
            if self.data["set"]["charging_ev"] != -1:
                # Altes EV merken
                self.data["set"]["charging_ev_prev"] = self.data["set"][
                    "charging_ev"]
                pub.pub(
                    "openWB/set/chargepoint/" + str(self.cp_num) +
                    "/set/charging_ev_prev",
            ...
            if self.data["set"]["charging_ev_prev"] != -1:
                ...
                data.data.pv_data["all"].reset_switch_on_off(
                    self, data.data.ev_data[
                        "ev" + str(self.data["set"]["charging_ev_prev"])])
                # Abstecken
                if self.data["get"]["plug_state"] is False:
                    # Standardprofil nach Abstecken laden
                    if data.data.ev_data[
                            "ev" + str(self.data["set"]["charging_ev_prev"]
                                       )].charge_template.data["load_default"]:
                        self.data["config"]["ev"] = 0
                        pub.pub(
                            "openWB/set/chargepoint/" + str(self.cp_num) +
                            "/config/ev", 0)
                    # Ladepunkt nach Abstecken sperren
                    if data.data.ev_data["ev" + str(
                            self.data["set"]["charging_ev_prev"]
                    )].charge_template.data["disable_after_unplug"]:
                    ...
                    pub.pub(
                        "openWB/set/chargepoint/" + str(self.cp_num) +
                        "/set/plug_time", "0")
                    self.data["set"]["phases_to_use"] = self.data["get"][
                        "phases_in_use"]
                    pub.pub(
                        "openWB/set/chargepoint/" + str(self.cp_num) +
                        "/set/phases_to_use",
            ...
            pub.pub(
                "openWB/set/chargepoint/" + str(self.cp_num) + "/set/current",
                0)
            self.data["set"]["energy_to_charge"] = 0
            pub.pub(
                "openWB/set/chargepoint/" + str(self.cp_num) +
                "/set/energy_to_charge", 0)
            return -1, message
        except Exception as e:
            log.MainLogger().exception("Fehler in der Ladepunkt-Klasse von " +
                                       str(self.cp_num))
    ...
    def initiate_control_pilot_interruption(self):
        ...
        try:
            ...
            if self.data["config"][
                    "control_pilot_interruption_hw"] and charging_ev.ev_template.data[
                        "control_pilot_interruption"]:
                # Wird die Ladung gestartet?
                if self.set_current_prev == 0 and self.data["set"][
                        "current"] != 0:
                    selected = self.data["config"]["connection_module"][
                        "selected"]
                    config = self.data["config"]["connection_module"][
                        "config"][selected]
                    ...
                    log.MainLogger().info("LP " + str(self.cp_num) + ": " +
                                          message)
    ...
    def initiate_phase_switch(self):
        ...
        try:
            ...
            if charging_ev.data["control_parameter"][
                    "timestamp_perform_phase_switch"] != "0":
                phase_switch_pause = charging_ev.ev_template.data[
                    "phase_switch_pause"]
                ...
                if timecheck.check_timestamp(
                    ...
                    if charging_ev.data["control_parameter"]["phases"] == 3:
                        data.data.pv_data["all"].data["set"][
                            "reserved_evu_overhang"] -= charging_ev.data[
                                "control_parameter"][
                                    "required_current"] * 3 * 230
                    ...
                    if self.data["set"]["phases_to_use"] == 3:
                        ...
                        raise ValueError(
                            ...
                            " ist keine gültige Phasenzahl (1/3).")
                    self.data["get"]["state_str"] = message
                return
            # Wenn noch kein Logeintrag erstellt wurde, wurde noch nicht geladen und die Phase kann noch umgeschaltet werden.
            if charging_ev.ev_template.data[
                    "prevent_switch_stop"] is False or self.data["set"]["log"][
                        "charged_since_plugged_counter"] == 0:
                ...
                if "phases_to_use" not in self.data["set"]:
                    ...
                    self.data["set"]["phases_to_use"] = charging_ev.data[
                ...
                if (self.data["set"]["phases_to_use"] !=
                        charging_ev.data["control_parameter"]["phases"] and
                        self.data["set"]["log"]["charged_since_mode_switch"] >
                        0 or self.data["get"]["phases_in_use"] !=
                        charging_ev.data["control_parameter"]["phases"] and
                        self.data["set"]["log"]["charged_since_mode_switch"]
                        == 0):
                    ...
                    if charging_ev.data["control_parameter"][
                        ...
                        if self.data["config"]["auto_phase_switch_hw"]:
                            selected = self.data["config"][
                                "connection_module"]["selected"]
                            config = self.data["config"]["connection_module"][
                                "config"][selected]
                            charge_state = self.data["get"]["charge_state"]
                            phase_switch.thread_phase_switch(
                                self.cp_num, selected, config,
                                charging_ev.data["control_parameter"]
                                ["phases"], charging_ev.ev_template.
                                data["phase_switch_pause"], charge_state)
                            log.MainLogger().debug(
                                "start phase switch phases_to_use " +
                                str(self.data["set"]["phases_to_use"]) +
                                "control_parameter phases " +
                                str(charging_ev.data["control_parameter"]
                                    ["phases"]))
                            # 1 -> 3
                            if charging_ev.data["control_parameter"][
                                    "phases"] == 3:
                                message = "Umschaltung von 1 auf 3 Phasen."
                                log.MainLogger().info("LP " +
                                                      str(self.cp_num) + ": " +
                                                      message)
                                self.data["get"]["state_str"] = message
                                ...
                                pub.pub(
                                    ...
                                    charging_ev.data["control_parameter"]
                                    ["timestamp_perform_phase_switch"])
                            elif charging_ev.data["control_parameter"][
                                    "phases"] == 1:
                                message = "Umschaltung von 3 auf 1 Phase."
                                # Timestamp für die Durchführungsdauer
                                charging_ev.data["control_parameter"][
                            ...
                            log.MainLogger().error(
                                "Phasenumschaltung an Ladepunkt" +
                                str(self.cp_num) +
            ...
            if self.data["set"]["phases_to_use"] != charging_ev.data[
                    "control_parameter"]["phases"]:
                pub.pub(
                    "openWB/set/chargepoint/" + str(self.cp_num) +
                    "/set/phases_to_use",
    ...
    def get_phases(self, mode):
        ...
        try:
            ...
            if charging_ev.ev_template.data["max_phases"] <= config[
                    "connected_phases"]:
                phases = charging_ev.ev_template.data["max_phases"]
            else:
                phases = config["connected_phases"]
            log.MainLogger().debug("phases1 " + str(phases))
            chargemode_phases = data.data.general_data[
                "general"].get_phases_chargemode(mode)
            ...
            if chargemode_phases == 0:
                ...
                charging_ev.data["control_parameter"]["phases"] = self.data[
                    "get"]["phases_in_use"]
                if charging_ev.data["control_parameter"][
                        "timestamp_perform_phase_switch"] == "0":
            ...
            log.MainLogger().debug("chargemode_phases " +
                                   str(chargemode_phases))
            log.MainLogger().debug("phases " + str(phases))
            # Wenn noch kein Logeintrag erstellt wurde, wurde noch nicht geladen und die Phase kann noch umgeschaltet werden.
            if charging_ev.ev_template.data[
                    "prevent_switch_stop"] and self.data["set"]["log"][
                        "charged_since_plugged_counter"] != 0:
                log.MainLogger().info(
                    "Phasenumschaltung an Ladepunkt" + str(self.cp_num) +
                    " nicht möglich, da bei EV " + str(charging_ev.ev_num) +
