...
def thread_phase_switch(cp_num, selected, config, phases_to_use, duration,
                        charge_state):
    ...
    try:
        ...
        phase_switch_threads = {
            t: phase_switch_threads[t]
            for t in phase_switch_threads
            if phase_switch_threads[t].is_alive()
        }
        ...
        if "thread_cp" + str(cp_num) not in phase_switch_threads:
            ...
            phase_switch_threads["thread_cp" + str(cp_num)] = threading.Thread(
                ...
                args=(selected, config, phases_to_use, duration, charge_state))
            phase_switch_threads["thread_cp" + str(cp_num)].start()
            log.MainLogger().debug("Thread zur Phasenumschaltung an LP" +
                                   str(cp_num) + " gestartet.")
    except Exception as e:
        log.MainLogger().exception("Fehler im Phasenumschaltungs-Modul")


def _perform_phase_switch(selected, config, phases_to_use, duration,
                          charge_state):
    ...
    try:
        ...
        if selected == "external_openwb":
            ...
            ip_address = config["ip_address"]
            id = config["id"]
            ip_evse.perform_phase_switch(ip_address, id, duration,
                                         phases_to_use)
