...
class ev:
    ...
    def __init__(self, index):
        try:
            ...
            log.MainLogger().exception("Fehler im ev-Modul " +
                                       str(self.ev_num))
    ...
    def reset_ev(self):
        ...
        """
        try:
            log.MainLogger().debug("EV " + str(self.ev_num) +
                                   " zurueckgesetzt.")
            pub.pub(
    ...
    def get_required_current(self, charged_since_mode_switch):
        ...
        state = True
        try:
            if self.charge_template.data["chargemode"][
                    "selected"] == "scheduled_charging":
                required_current, submode, message = self.charge_template.scheduled_charging(
                    self.data["get"]["soc"], self.ev_template,
            ...
            log.MainLogger().exception("Fehler im ev-Modul " +
                                       str(self.ev_num))
            return False, "ein interner Fehler aufgetreten ist: " + traceback.format_exc(
            ), "stop", 0
    ...
    def check_state(self, required_current, set_current, charge_state):
        ...
        try:
            ...
            if self.data["control_parameter"][
                    "chargemode"] != self.charge_template.data["chargemode"][
                        "selected"]:
                mode_changed = True
            ...
            if self.data["control_parameter"][
                ...
                if ((self.charge_template.data["chargemode"]["selected"]
                     == "pv_charging"
                     or self.charge_template.data["chargemode"]["selected"]
                     == "scheduled_charging") and
                    ((self.data["control_parameter"]["submode"]
                      == "pv_charging"
                      or self.data["control_parameter"]["chargemode"]
                      == "pv_charging") and set_current >
                     self.data["control_parameter"]["required_current"])):
                    current_changed = False
                else:
                    current_changed = True

            log.MainLogger().debug("Aenderung der Sollstromstaerke :" +
                                   str(current_changed) +
                                   ", Aenderung des Lademodus :" +
                                   str(mode_changed))
    ...
    def set_control_parameter(self, submode, required_current):
        ...
        try:
            ...
            pub.pub(
                "openWB/set/vehicle/" + str(self.ev_num) +
                "/control_parameter/chargemode",
                self.charge_template.data["chargemode"]["selected"])
            self.data["control_parameter"]["prio"] = self.charge_template.data[
                "prio"]
            pub.pub(
                "openWB/set/vehicle/" + str(self.ev_num) +
        ...
        try:
            ...
            if phases != 0:
                ...
                if required_current != 0:
                    if pv is False:
                        ...
                        min_current = self.data["control_parameter"][
                            "required_current"]
                    if required_current < min_current:
                        required_current = min_current
                    else:
                        if phases == 1:
                            max_current = self.ev_template.data[
            ...
            if required_current != required_current_prev:
                log.MainLogger().debug(
                    "Anpassen der Sollstromstaerke an EV-Vorgaben. Sollstromstarke: "
                    + str(required_current_prev) + " neue Sollstromstarke: " +
                    str(required_current))
    ...
    def auto_phase_switch(self, cp_num, current, phases_in_use, current_get):
        ...
        try:
            ...
            if self.ev_template.data[
                    "prevent_switch_stop"] is False and self.data[
                        "control_parameter"][
                            "timestamp_perform_phase_switch"] == "0":
                ...
                if phases_in_use == 1:
                    ...
                    # Wenn im einphasigen Laden die Maximalstromstärke erreicht wird und der Timer noch nicht läuft, Timer für das Umschalten auf 3 Phasen starten.
                    elif (self.data["control_parameter"]
                          ["timestamp_auto_phase_switch"] == "0"
                          and max(current_get) >=
                          (self.ev_template.data["max_current_one_phase"] -
                           self.ev_template.data["nominal_difference"])):
                        ...
                        log.MainLogger().info("LP " + str(cp_num) + ": " +
                                              message)
                ...
                # 3 -> 1
                    ...
                    if (self.data["control_parameter"]
                        ["timestamp_auto_phase_switch"] != "0" and all(
                            (current <=
                             (self.data["control_parameter"]
                              ["required_current"] +
                              self.ev_template.data["nominal_difference"])
                             or current <=
                             self.ev_template.data["nominal_difference"])
                            for current in current_get)):
                        if timecheck.check_timestamp(
                                self.data["control_parameter"]
                            ["timestamp_auto_phase_switch"],
                            (16 - pv_config["phase_switch_delay"]) *
                                60) is False:
                            phases_to_use = 1
                            # Nach dem Umschalten wieder mit Maximalstromstärke laden.
                            current = self.ev_template.data[
                                "max_current_one_phase"]
                            self.data["control_parameter"][
                                "timestamp_auto_phase_switch"] = "0"
                            pub.pub(
                    ...
                    # Wenn der Timer läuft und mit mehr als Minimalstromstärke geladen wird, Timer stoppen.
                            ...
                            "timestamp_auto_phase_switch"] != "0" and any(
                                current >
                                (self.data["control_parameter"]
                                 ["required_current"] +
                                 self.ev_template.data["nominal_difference"])
    ...
    def reset_phase_switch(self):
        ...
        if self.data["control_parameter"]["timestamp_auto_phase_switch"] != "0":
            ...
            if self.data["control_parameter"]["phases"] == 3:
                data.data.pv_data["all"].data["set"]["reserved_evu_overhang"] -= self.ev_template.data["max_current_one_phase"] * \
                    230 - \
                    self.data["control_parameter"]["required_current"] * 3 * 230
                log.MainLogger().debug(
                ...
                data.data.pv_data["all"].data["set"]["reserved_evu_overhang"] -= self.data["control_parameter"]["required_current"] * \
                    3 * 230 - \
                    self.ev_template.data["max_current_one_phase"] * 230
                log.MainLogger().debug(
                    "Zuruecksetzen der reservierten Leistung fuer die Phasenumschaltung. reservierte Leistung: "
                    + str(data.data.pv_data["all"].data["set"]
                          ["reserved_evu_overhang"]))
    ...
    def instant_charging(self, soc, amount):
        ...
        try:
            ...
            if data.data.optional_data["optional"].data["et"]["active"]:
                if data.data.optional_data[
                        "optional"].et_price_lower_than_limit() is False:
            ...
            if instant_charging["limit"]["selected"] == "none":
                ...
                if soc < instant_charging["limit"]["soc"]:
                    return instant_charging[
                        "current"], "instant_charging", message
                else:
            ...
            if soc < pv_charging["max_soc"]:
                if pv_charging["min_soc"] != 0:
                    if soc < pv_charging["min_soc"]:
                        return pv_charging[
                            "min_soc_current"], "instant_charging", message
                if pv_charging["min_current"] == 0:
                    ...
                    # Min PV
                    return pv_charging[
                        "min_current"], "instant_charging", message
            else:
    ...
    def scheduled_charging(self, soc, ev_template, phases):
        ...
        try:
            ...
            for plan in self.data["chargemode"]["scheduled_charging"]["plans"]:
                if self.data["chargemode"]["scheduled_charging"]["plans"][
                        plan]["active"]:
                    try:
                        if soc < self.data["chargemode"]["scheduled_charging"][
                                "plans"][plan]["soc"]:
                            if phases == 1:
                                max_current = ev_template.data[
                                    "max_current_one_phase"]
                            else:
                                max_current = ev_template.data[
                                    "max_current_multi_phases"]
                            ...
                            required_wh = (
                                (self.data["chargemode"]["scheduled_charging"]
                                 ["plans"][plan]["soc"] - soc) /
                                100) * battery_capacity * 1000
                            duration = required_wh / (available_current * 230)
                            start, remaining_time = timecheck.check_duration(
                                self.data["chargemode"]["scheduled_charging"]
                                ["plans"][plan], duration)
                            # Erster Plan
                            if (start == plan_data["start"] and
                                    remaining_time <= smallest_remaining_time
                                ) or start > plan_data["start"]:
                ...
                if start == -1:
                    ...
                    log.MainLogger().debug("Verwendeter Plan: " +
                                           str(plan_data))
                    ...
                    for plan in self.data["chargemode"]["scheduled_charging"]:
                        if plan == plan_data["plan"]:
                            current_plan = self.data["chargemode"][
                                "scheduled_charging"]["plans"][plan]
                            break
                    else:
                        return 0, "stop", "Keine Ladung, da ein interner Fehler aufgetreten ist: " + traceback.format_exc(
                        )
                    if plan_data["start"] == 1:  # Ladung sollte jetzt starten
                        message = "Zielladen mit " + str(
                            plan_data["available_current"]
                        ) + "A, um einen SoC von " + str(
                            current_plan["soc"]) + "%% um " + str(
                                current_plan["time"]) + " zu erreichen."
                        return plan_data[
                            "available_current"], "instant_charging", message
                    # weniger als die berechnete Zeit verfügbar
                        ...
                        if required_current >= plan_data["max_current"]:
                            plan_data["available_current"] = plan_data[
                                "max_current"]
                        else:
                        ...
                        if timecheck.check_timeframe(current_plan, 24):
                            ...
                            if data.data.optional_data["optional"].data["et"][
                                    "active"]:
                                hourlist = data.data.optional_data[
                                    "optional"].et_get_loading_hours(duration)
