            ...
            for chargepoint in data.data.cp_data:
                try:
                    if "cp" in chargepoint:
                        data.data.cp_data[
                            chargepoint].template = data.data.cp_template_data[
                                "cpt" + str(data.data.cp_data[chargepoint].
                                            data["config"]["template"])]
                        # Status zurücksetzen (wird jeden Zyklus neu ermittelt)
                        data.data.cp_data[chargepoint].data["get"][
                            "state_str"] = None
                except Exception as e:
                    log.MainLogger().exception(
                        "Fehler im Prepare-Modul fuer Ladepunkt " +
                        str(chargepoint))
            ...
            for vehicle in data.data.ev_data:
                try:
                    ...
                    if data.data.general_data["general"].data[
                            "chargemode_config"]["individual_mode"]:
                        data.data.ev_data[
                            vehicle].charge_template = data.data.ev_charge_template_data[
                                "ct" + str(data.data.ev_data[vehicle].
                                           data["charge_template"])]
                    else:
                        data.data.ev_data[
                            vehicle].charge_template = data.data.ev_charge_template_data[
                                "ct0"]
                    ...
                    data.data.ev_data[
                        vehicle].ev_template = data.data.ev_template_data[
                            "et" + str(data.data.ev_data[vehicle].
                                       data["ev_template"])]
                except Exception as e:
                    log.MainLogger().exception(
                        "Fehler im Prepare-Modul fuer EV " + str(vehicle))
                ...
                if "cp" in cp_item:
                    ...
                    if vehicle != -1:
                        ...
                        if (cp.data["set"]["charging_ev"] == vehicle or
                            ...
                            pub.pub(
                                "openWB/set/chargepoint/" + str(cp.cp_num) +
                                "/set/charging_ev", vehicle)
                            charging_ev.ev_template.data = charging_ev.data[
                                "set"]["ev_template"]
                            ...
                            if cp.data["set"]["log"][
                                    "counter_at_plugtime"] == 0 or cp.data[
                                        "set"]["log"][
                                            "counter_at_plugtime"] == cp.data[
                                                "get"]["counter"]:
                                cp.data["set"]["charging_ev"] = vehicle
                                pub.pub(
                                    "openWB/set/chargepoint/" +
                                    str(cp.cp_num) + "/set/charging_ev",
                                    vehicle)
                                cp.data["set"][
                                    "charging_ev_data"] = charging_ev
                                pub.pub(
                                    "openWB/set/chargepoint/" +
                                    str(cp.cp_num) +
                                    "/set/change_ev_permitted", [True, ""])
                                charging_ev.data["set"][
                                    "ev_template"] = charging_ev.ev_template.data
                                pub.pub(
                                    "openWB/set/vehicle/" +
                                    str(charging_ev.ev_num) +
                                    "/set/ev_template",
                                    charging_ev.data["set"]["ev_template"])
                            else:
                                ...
                                if cp.data["set"]["charging_ev"] != -1:
                                    ...
                                    pub.pub(
                                        "openWB/set/chargepoint/" +
                                        str(cp.cp_num) +
                                        "/set/charging_ev_prev", -1)
                                else:
                                    raise ValueError(
                                        "Wenn kein aktuelles und kein vorheriges Ev zugeordnet waren, sollte noch nicht geladen worden sein."
                                    )
                                ...
                                pub.pub(
                                    "openWB/set/chargepoint/" +
                                    str(cp.cp_num) +
                                    "/set/change_ev_permitted", [
                                        False,
                                        "Das Fahrzeug darf nur geändert werden, wenn noch nicht geladen wurde. Bitte abstecken, dann wird das gewählte Fahrzeug verwendet."
                                    ])
                        ...
                        log.MainLogger().debug(
                            "Ladepunkt " + str(cp.cp_num) + ", EV: " +
                            cp.data["set"]["charging_ev_data"].data["name"] +
                            " (EV-Nr." + str(vehicle) + ")")
                        ...
                        if current_changed:
                            log.MainLogger().debug(
                                "LP" + str(cp.cp_num) +
                                " : Da sich die Stromstärke geändert hat, muss der Ladepunkt im Algorithmus neu priorisiert werden."
                            )
                            data.data.pv_data["all"].reset_switch_on_off(
                                cp, charging_ev)
                            charging_ev.reset_phase_switch()
                            if max(cp.data["get"]
                                   ["current"]) > charging_ev.ev_template.data[
                                       "nominal_difference"]:
                        ...
                        if mode_changed and cp.data["get"][
                                "charge_state"] and state:
                        ...
                        if state is False:
                            if cp.data["set"]["charging_ev"] != -1:
                                # Altes EV merken
                                cp.data["set"]["charging_ev_prev"] = cp.data[
                                    "set"]["charging_ev"]
                                pub.pub(
                                    "openWB/set/chargepoint/" +
                                    str(cp.cp_num) + "/set/charging_ev_prev",
                                    cp.data["set"]["charging_ev_prev"])
                            cp.data["set"]["charging_ev"] = -1
                            pub.pub(
                                "openWB/set/chargepoint/" + str(cp.cp_num) +
                                "/set/charging_ev", -1)
                            log.MainLogger().debug(
                                "EV" + str(charging_ev.ev_num) +
                                ": Lademodus " +
                                str(charging_ev.charge_template.
                            ...
                            if (charging_ev.data["control_parameter"]
                                ["timestamp_switch_on_off"] != "0"
                                    and cp.data["get"]["charge_state"] is False
                                    and data.data.pv_data["all"].data["set"]
                                ["overhang_power_left"] == 0):
                                log.MainLogger().error(
                                    "Reservierte Leistung kann nicht 0 sein.")

                            log.MainLogger().debug(
                                "EV" + str(charging_ev.ev_num) +
                                ": Theroretisch benötigter Strom " +
                                str(required_current) + "A, Lademodus " +
                                str(charging_ev.charge_template.
                                    data["chargemode"]["selected"]) +
                                ", Submodus: " +
                                str(charging_ev.data["control_parameter"]
                                    ["submode"]) + ", Phasen: " + str(phases) +
                                ", Prioritaet: " +
                                str(charging_ev.charge_template.data["prio"]) +
                                ", max. Ist-Strom: " +
                        ...
                        self._pub_connected_vehicle(
                            data.data.ev_data["ev" +
                                              str(cp.data["config"]["ev"])],
                            cp)
                    if message is not None and cp.data["get"][
                            "state_str"] is None:
                        log.MainLogger().info("LP " + str(cp.cp_num) + ": " +
                                              message)
                        cp.data["get"]["state_str"] = message
            except Exception as e:
                log.MainLogger().exception(
                    "Fehler im Prepare-Modul fuer Ladepunkt " + str(cp_item))
    ...
    def _pub_connected_vehicle(self, vehicle, chargepoint):
        ...
        try:
            ...
            soc_obj = {
                ...
                "range_unit":
                data.data.general_data["general"].data["range_unit"],
            ...
            info_obj = {"id": vehicle.ev_num, "name": vehicle.data["name"]}
            if vehicle.charge_template.data["chargemode"][
                    "selected"] == "time_charging":
                current_plan = vehicle.charge_template.data["chargemode"][
                    "current_plan"]
            elif vehicle.charge_template.data["chargemode"][
            ...
            config_obj = {
                "charge_template":
                vehicle.charge_template.ct_num,
                "ev_template":
                vehicle.ev_template.et_num,
                "chargemode":
                vehicle.charge_template.data["chargemode"]["selected"],
                "priority":
                vehicle.charge_template.data["prio"],
                "current_plan":
                current_plan,
                "average_consumption":
                vehicle.ev_template.data["average_consump"]
            }
            if soc_config_obj != chargepoint.data["get"]["connected_vehicle"][
                    "soc_config"]:
                pub.pub(
                    "openWB/chargepoint/" + str(chargepoint.cp_num) +
            ...
            if config_obj != chargepoint.data["get"]["connected_vehicle"][
                    "config"]:
                pub.pub(
                    "openWB/chargepoint/" + str(chargepoint.cp_num) +
