...
def loadmanagement_for_cp(chargepoint, required_power, required_current,
                          phases):
    ...
    try:
        ...
        if phases == 3:
            ...
            if chargepoint.data["config"]["phase_1"] == 0:
                ...
                raise ValueError(
                    ...
                    "ist keine gueltige Zahl für die angeschlossene Phase (0-3"
                )
        ...
        for counter in counters[:-1]:
            try:
                loadmanagement, overshoot, phase = _check_max_current(
        ...
        loadmanagement = _loadmanagement_for_evu(required_power,
        ...
        data.data.counter_data["all"].data["set"][
            "loadmanagement_active"] = loadmanagement_all_conditions
    ...
    for child in child["children"]:
        try:
            ...
            log.MainLogger().exception(
                "Fehler im Lastmanagement-Modul fuer Zaehler " + str(child))
...
def _loadmanagement_for_evu(required_power, required_current_phases, phases,
                            offset):
    ...
    try:
        ...
        loadmanagement, overshoot, phase = _check_unbalanced_load(
...
def _check_max_power(required_power, offset):
    ...
    if offset:
        ...
        offset_power = 0
    try:
        consumption_left = data.data.counter_data[evu_counter].data["set"][
            "consumption_left"] - required_power - offset_power
        data.data.counter_data[evu_counter].data["set"][
            "consumption_left"] -= required_power
        # Float-Ungenauigkeiten abfangen
        if consumption_left >= -0.01:
            return False, data.data.counter_data[evu_counter].data["set"][
                "consumption_left"] - 300
        else:
            return True, data.data.counter_data[evu_counter].data["set"][
                "consumption_left"] - 300
    except Exception as e:
        log.MainLogger().exception("Fehler im Lastmanagement-Modul")
        return 0, False
...
def _check_max_current(counter, required_current_phases, phases, offset):
    ...
    try:
        for phase in range(3):
            ...
            if current_used[phase] < data.data.counter_data[counter].data[
                    "config"]["max_current"][phase] - offset_current:
                loadmanagement = False
            else:
                if (current_used[phase] -
                    (data.data.counter_data[counter].data["config"]
                     ["max_current"][phase] -
        ...
        if max_current_overshoot != 0:
            ...
            if offset:
                ...
                log.MainLogger().warning(
                    "Benoetigte Stromstaerke " +
                    str(current_used.index(max(current_used))) +
                    " ueberschreitet unter Beachtung des Offsets die zulaessige Stromstaerke an Phase "
                    + str(current_used.index(max(current_used))) + " um " +
                    str(max_current_overshoot) + "A.")
            else:
                log.MainLogger().debug("Strom " + str(current_used))
                log.MainLogger().warning(
                    "Benoetigte Stromstaerke " +
                    ...
                    + str(current_used.index(max(current_used))) + " um " +
                    str(max_current_overshoot) + "A.")
        data.data.counter_data[counter].data["set"][
        ...
        return loadmanagement, max_current_overshoot + (
            300 / 230 / phases), current_used.index(max(current_used))
    except Exception as e:
        log.MainLogger().exception("Fehler im Lastmanagement-Modul")
        return False, 0, 0
...
def _check_unbalanced_load(current_used, offset):
    ...
    """
    if data.data.general_data["general"].data["chargemode_config"][
            "unbalanced_load"]:
        max_current_overshoot = 0
        if offset:
            offset_current = 1
        ...
        try:
            ...
            ) <= data.data.general_data["general"].data["chargemode_config"][
                    "unbalanced_load_limit"] - offset_current:
                return False, None, 0
            else:
                max_current_overshoot = (
                    max_current -
                    min_current) - data.data.general_data["general"].data[
                        "chargemode_config"]["unbalanced_load_limit"]
                log.MainLogger().warning("Schieflast wurde ueberschritten.")
                return True, max_current_overshoot + 1, current_used.index(
                    max_current) + 1
        except Exception as e:
            log.MainLogger().exception("Fehler im Lastmanagement-Modul")
            return False, 0, 0
