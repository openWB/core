...
class counterAll:
    ...
    def get_evu_counter(self):
        try:
            ...
            log.MainLogger().exception(
                "Fehler in der allgemeinen Zaehler-Klasse")

    def put_stats(self):
        try:
            pub.pub("openWB/set/counter/set/loadmanagement_active",
                    self.data["set"]["loadmanagement_active"])
        except Exception as e:
            log.MainLogger().exception(
                "Fehler in der allgemeinen Zaehler-Klasse")
    ...
    def calc_home_consumption(self):
        ...
        """
        try:
            evu = data.data.counter_data[
                self.get_evu_counter()].data["get"]["power_all"]
            ...
            pub.pub("openWB/set/counter/set/home_consumption",
                    self.data["set"]["home_consumption"])
    ...
    def calc_daily_yield_home_consumption(self):
        ...
        """
        try:
            evu_imported = data.data.counter_data[
                self.get_evu_counter()].data["get"]["daily_yield_import"]
            evu_exported = data.data.counter_data[
                self.get_evu_counter()].data["get"]["daily_yield_export"]
            if len(data.data.pv_data) > 1:
                pv = data.data.pv_data["all"].data["get"]["daily_yield"]
            else:
                pv = 0
            if len(data.data.bat_data) > 1:
                bat_imported = data.data.bat_data["all"].data["get"][
                    "daily_yield_import"]
                bat_exported = data.data.bat_data["all"].data["get"][
                    "daily_yield_export"]
            else:
                bat_imported = 0
                bat_exported = 0
            ...
            pub.pub("openWB/set/counter/set/daily_yield_home_consumption",
                    daily_yield_home_consumption)
            self.data["set"][
                "daily_yield_home_consumption"] = daily_yield_home_consumption
        except Exception as e:
            log.MainLogger().exception(
                "Fehler in der allgemeinen Zaehler-Klasse")
        ...
        try:
            if counter == self.get_evu_counter():
                ...
                counter_object = self._look_for_object(
                    self.data["get"]["hierarchy"][0], "counter", counter[7:])
    ...
    def get_counters_to_check(self, chargepoint):
        ...
        try:
            ...
            self._look_for_object(self.data["get"]["hierarchy"][0], "cp",
                                  chargepoint.cp_num)
        ...
        try:
            ...
            for child in child["children"]:
                try:
                    ...
                    log.MainLogger().exception(
                        "Fehler in der allgemeinen Zaehler-Klasse für " +
                        child)
            else:
                return False
        except Exception as e:
    ...
    def hierarchy_add_item_aside(self, id, lower_level):
        ...
        """
        return self._add_item_aside(self.data["get"]["hierarchy"][0], id,
                                    lower_level)
            ...
            for child in upper_level["children"]:
                try:
                    if lower_level in child["id"]:
                        upper_level["children"].append({
                            "id": id,
                            "children": []
                        })
                        pub.pub(
                            "openWB/set/counter/get/hierarchy",
                            data.data.counter_data["all"].data["get"]
                            ["hierarchy"])
                        return True
                    else:
                        if len(child["children"]) != 0:
                            added = self._add_item_aside(
                                child, id, lower_level)
                            return added
                except Exception as e:
                    log.MainLogger().exception(
                        "Fehler in der allgemeinen Zaehler-Klasse für " +
            ...
            for child in upper_level["children"]:
                try:
                    if id in child["id"]:
                        ...
                        if len(child["children"]) != 0:
                            removed = self._remove_item(
                                child, id, keep_children)
                            return removed
                except Exception as e:
                    log.MainLogger().exception(
                        "Fehler in der allgemeinen Zaehler-Klasse für " +
    ...
    def hierarchy_add_item_below(self, id, below):
        ...
        try:
            if below in self.data["get"]["hierarchy"][0]["id"]:
                self.data["get"]["hierarchy"][0]["children"].append({
                    "id":
                    id,
                    "children": []
                })
                ...
                if len(self.data["get"]["hierarchy"][0]["children"]) != 0:
                    added = self._add_item_below(
                        self.data["get"]["hierarchy"][0], id, below)
                    return added
        except Exception as e:
            log.MainLogger().exception(
                "Fehler in der allgemeinen Zaehler-Klasse")
...
class counter:
    ...
    def __init__(self, index):
        try:
            ...
            log.MainLogger().exception("Fehler in der Zaehler-Klasse von " +
                                       self.counter_num)
    ...
    def setup_counter(self):
        ...
        try:
            ...
            if self.counter_num == 0:
                ...
                if self.data["get"]["power_all"] is None or self.data["get"][
                        "current"] is None:
                    data.data.counter_data["all"].data["set"][
                ...
                if self.data["get"]["power_all"] > 0:
                    self.data["set"]["consumption_left"] = self.data["config"][
                        "max_consumption"] - self.data["get"]["power_all"]
                else:
                    self.data["set"]["consumption_left"] = self.data["config"][
                        "max_consumption"]
                log.MainLogger().debug(
                    str(self.data["set"]["consumption_left"]) +
    ...
    def put_stats(self):
        try:
            pub.pub("openWB/set/counter/0/set/consumption_left",
                    self.data["set"]["consumption_left"])
            log.MainLogger().debug(
                str(self.data["set"]["consumption_left"]) +
                "W verbleibende EVU-Bezugs-Leistung")
        except Exception as e:
            log.MainLogger().exception("Fehler in der Zaehler-Klasse von " +
                                       self.counter_num)
