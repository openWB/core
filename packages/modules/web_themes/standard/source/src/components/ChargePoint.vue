...
const targetSoc = computed<number | undefined>(() => {
  const chargeMode = mqttStore.chargePointConnectedVehicleChargeMode(props.chargePointId).value;
  if (chargeMode === 'scheduled_charging') {
    return mqttStore.vehicleScheduledChargingTarget(props.chargePointId).value?.soc;
  } else if (chargeMode === 'instant_charging') {
    return mqttStore.chargePointConnectedVehicleInstantChargeLimitSoC(props.chargePointId)?.value;
  } else {
    return undefined;
  }
});
