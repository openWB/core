<template>
	<div class="container-fluid p-0 m-0">
		<ConfigItem title="Beginn" :fullwidth="true">
			<TimeInput v-model="plan.time[0]" />
		</ConfigItem>

		<ConfigItem title="Ende" :fullwidth="true">
			<TimeInput v-model="plan.time[1]" />
		</ConfigItem>
		...
		<div class="row">
			<div class="col d-flex justify-content-end">
				<button class="btn btn-danger" @click="$emit('deletePlan', planId)">
					Löschen
				</button>
				<button class="btn btn-warning ms-1" @click="$emit('abort')">
					Abbrechen
				</button>
				<button
