...
class module(set_values.set_values):
    ...
    def read(self):
        ...
        try:
            ...
            log.log_exception_comp(
                e, self.ramdisk, "Counter"+str(self.counter_num))
    ...
    def _read_version0(self):
        ...
        try:
            ...
            try:
                ...
                value2 = resp.registers[1]
                all = format(value1, '04x') + format(value2, '04x')
                power_all = int(struct.unpack(
                    '>i', all.decode('hex'))[0]) / 100
            except Exception as e:
                log.log_exception_comp(
                    e, self.ramdisk, "Counter"+str(self.counter_num))
                power_all = 0
        ...
        try:
            ...
            try:
                ...
                all = format(resp.registers[0], '04x') + \
                    format(resp.registers[1], '04x')
                current1 = abs(
                    float(struct.unpack('>i', all.decode('hex'))[0]) / 10000)
            ...
            if self.ramdisk:
                imported, exported = simcount.sim_count(
                    power_all, ramdisk=True, pref="bezug")
            else:
                imported, exported = simcount.sim_count(
                    power_all, topic="openWB/set/counter/"+str(self.counter_num)+"/", data=self.data["simulation"])
    ...
    def _read_sdm(self):
        ...
        try:
            ...
            try:
                ...
                current1 = abs(
                    float(struct.unpack('>f', struct.pack('>HH', *resp.registers))[0]))
