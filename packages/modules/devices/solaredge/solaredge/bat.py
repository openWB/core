...
class SolaredgeBat(AbstractBat):
    ...
    def set_power_limit(self, setpoint: SetPoint) -> None:
        ...
        if setpoint.power_limit is None:  # No Bat Control should be used.
        ...
        elif setpoint.power_limit <= 0:  # Limit Discharge Mode should be used.
            if (values[Registers.STORAGE_CONTROL_MODE] == CONTROL_MODE_REMOTE and
                ...
                if discharge_limit not in range(int(abs(setpoint.power_limit)) - 10,
                                                int(abs(setpoint.power_limit)) + 10):
                    # Send Limit only if difference is more than 10W, needed with more than 1 battery.
                    values_to_write = {
                        Registers.REMOTE_CONTROL_DISCHARGE_LIMIT: int(min(abs(setpoint.power_limit),
                                                                          MAX_CHARGEDISCHARGE_LIMIT))
                    }
                    self._write_registers(values_to_write, unit)
                    log.debug(f"Entlade-Limit Speicher{battery_index}: {int(abs(setpoint.power_limit))}W.")
                else:
                ...
                self._write_registers(values_to_write, unit)
                log.debug(f"Entlade-Limit aktiviert, Speicher{battery_index}: {int(abs(setpoint.power_limit))}W.")

        elif setpoint.power_limit > 0:  # Charge Mode should be used
            if (values[Registers.STORAGE_CONTROL_MODE] == CONTROL_MODE_REMOTE and
                ...
                if charge_limit not in range(int(abs(setpoint.power_limit)) - 10, int(abs(setpoint.power_limit)) + 10):
                    # Send Limit only if difference is more than 10W.
                    values_to_write = {
                        Registers.REMOTE_CONTROL_CHARGE_LIMIT: int(min(abs(setpoint.power_limit),
                                                                       MAX_CHARGEDISCHARGE_LIMIT))
                    }
                    self._write_registers(values_to_write, unit)
                    log.debug(f"Ladung Speicher{battery_index}: {int(abs(setpoint.power_limit))}W.")
                else:
                ...
                self._write_registers(values_to_write, unit)
                log.debug(f"Aktivierung Ladung Speicher{battery_index}: {int(abs(setpoint.power_limit))}W.")
