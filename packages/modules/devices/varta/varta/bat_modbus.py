...
class VartaBatModbus(AbstractBat):
    ...
    def set_power_limit(self, setpoint: SetPoint) -> None:
        ...
        if setpoint.power_limit is None:
            ...
            log.debug(f"Aktive Batteriesteuerung. Batterie darf mit {setpoint.power_limit} W entladen werden "
                      "für den Hausverbrauch")
            uint16_value = struct.unpack(">H", struct.pack(">h", setpoint.power_limit))[0]
            self.client.write_register(1074, uint16_value, data_type=ModbusDataType.INT_16, unit=unit)
            self.last_mode = 'discharge'
        else:
            # Das Register muss kontinuierlich geschrieben werden, da der Speicher
            # sonst nach 120s die Steuerung aufhebt.
            if setpoint.power_limit == 0:
                log.debug("Aktive Batteriesteuerung, Speicher wird auf Stop gesetzt.")
            else:
                log.debug(f"Aktive Batteriesteuerung, übergebene Leistung: {setpoint.power_limit}W. "
