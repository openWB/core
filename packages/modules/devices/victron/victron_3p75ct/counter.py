...
class VictronCounter(AbstractCounter):
    ...
    def update(self):
        ...
        with self.__udp_client:
            if energy_meter:
                powers = [
                    ...
                    for reg in [0x3082, 0x3086, 0x308A]] #4 byte abstand
                currents = [
                    self.__udp_client.read_holding_registers(reg, ModbusDataType.INT_16, unit=unit) / 100
                    for reg in [0x3041, 0x3049, 0x3051]] #8 byte abstand
                voltages = [
                    self.__udp_client.read_holding_registers(reg, ModbusDataType.INT_16, unit=unit) / 100
                    for reg in [0x3040, 0x3048, 0x3050]] #8 byte abstand 
                power_factors = [
        ...
        self.__udp_client.read_holding_registers_bulk()
