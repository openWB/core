...
def getString(param_id: str, brand: str) -> str:
    if brand == 'kia':
        if param_id == "app_id":
            ...
            paramStr = "cF5o4DiiQkaw5wsAkLzYIS:APA91bFB59MltBMK29zI0U2llq7khbB2jELkNFKMfBCH6KlCPL16pz_dG0fZ4ncvFn1IMT8nfojb83JyLiT_skBTXtClHhDCKeRbyPy3yQjCVRC3zTZt--wI7vv4jD9aknhHhiQsoZoU"
        elif param_id == "basic_token":
            paramStr = "Basic ZmRjODVjMDAtMGEyZi00YzY0LWJjYjQtMmNmYjE1MDA3MzBhOnNlY3JldA=="
        elif param_id == "stamp_pad":
            paramStr = "C0B4D5C7089D987F027C96015929C70FA9D2B2AA99530CFD017E4B243C4BA5C5DED96DEB128EEB5DD3963DFC12432C9073EF"
        elif param_id == "host":
    ...
    if brand == 'hyundai':
        if param_id == "app_id":
            ...
            paramStr = "Basic NmQ0NzdjMzgtM2NhNC00Y2YzLTk1NTctMmExOTI5YTk0NjU0OktVeTQ5WHhQekxwTHVvSzB4aEJDNzdXNlZYaG10UVI5aVFobUlGampvWTRJcHhzVg=="
        elif param_id == "stamp_pad":
            paramStr = "445B6846AFEF0D726646776865A650C9AEF98E51A474DCB7EC9B1B67D29C66EAAEF621CA02522A0B80A8087F7A3A7BB0F71B"
        elif param_id == "host":
...
def getHTTP(url: str = "", headers: str = "", cookies: str = "", timeout: int = 30, allow_redirects: bool = True) -> str:
    global last_cookies
    global last_url
    ...
    if response.status_code == 200 or response.status_code == 204:
        ...
        try:
            ...
            if response.status_code == 400 or response.status_code == 408 or response.status_code == 503:
                error_string = "[" + response_dict['resCode'] + "] " + response_dict['resMsg']
            else:
                error_string = "[" + response_dict['errCode'] + "] " + response_dict['errMsg']
        except:
            error_string = "[XXXX] Unidentified Error" + " " + response.text
        
        log.debug("kia.getHTTP:Request failed, StatusCode: " + str(response.status_code) + ', Error: ' + error_string)
        raise RuntimeError

    return ""


def putHTTP(url: str = "", data: Union[str, dict] = "", headers: str = "", cookies: str = "", timeout: int = 30) -> str:
    try:
    ...
    if response.status_code == 200 or response.status_code == 204:
        ...
        log.debug("kia.putHTTP:Request failed, StatusCode: " + str(response.status_code) + ', Error: ' + error_string)
        raise RuntimeError

    return ""
    
def deleteHTTP(url: str = "", headers: str = "", cookies: str = "", timeout: int = 30) -> None:
    try:
...
def postHTTP(url: str = "", data: Union[str, dict] = "", headers: str = "", cookies: str = "", timeout: int = 30, allow_redirects: bool = True) -> str:
    global last_cookies
    global last_url
    ...
    if response.status_code == 200 or response.status_code == 204:
        ...
        log.debug("kia.postHTTP:Request failed, StatusCode: " + str(response.status_code) + ', Error: ' + error_string)
        raise RuntimeError
...
def getHTTPCookies(url: str = "") -> dict:
    ...
    if response.status_code == 200:
        ...
        log.debug("kia.getHTTPCookies: Receiving cookies failed, StatusCode: " + str(response.status_code))
        raise RuntimeError
...
def loadToken(user_id: str, password: str, vehicle: int) -> dict:
    try:
        token_file = str(RAMDISK_PATH) + "/soc_kia_vehicle" + str(vehicle) + "_token"
...
def getCookies(brand: str) -> dict:
    ...
    try:
        url = getString("base_url", brand) + '/api/v1/user/oauth2/authorize?response_type=code&state=test&client_id=' + \
            getString("client_id", brand) + '&redirect_uri=' + getString("base_url", brand) + \
        ...
        data = {"lang": "en"}
        response = postHTTP(url=url, data=data, headers=headers, cookies=cookies)
        
        url = getString("base_url", brand) + '/api/v1/user/session'
        deleteHTTP(url=url, cookies=cookies)
    except Exception as err:
        log.debug("kia.getCookies: ", exc_info = True)
        raise
...
def getDeviceId(brand: str) -> dict:
    ...
    try:
        ...
        url = getString("base_url", brand) + '/api/v1/spa/notifications/register'
        data = {"pushRegId": getString("gcm_sender_id", brand), "pushType": "GCM", "uuid": str(uuid.uuid4())}
        headers = {
            'Authorization': '',
            'Ccsp-Device-Id': '',
        ...
        log.debug("kia.getDeviceId: Request device id failed: " + response, exc_info = True)
        raise   
    ...
    try:
        ...
        log.debug("kia.getDeviceId: Set version info failed: " + response, exc_info = True)
        raise  
...
def getAuthCode(username: str, password: str, brand: str, cookies: dict) -> str:
    global last_cookies
    ...
    try:
        ...
        url = 'https://eu-account.' + brand + '.com/auth/realms/eu' + brand + 'idm/protocol/openid-connect/auth?client_id=' + \
            getString("auth_client_id", brand) + '&scope=openid%20profile%20email%20phone&response_type=code&hkid_session_reset=true&redirect_uri=' + \
            getString("base_url", brand) + '/api/v1/user/integration/redirect/login&ui_locales=en&state=' + service_id + ':' + user_id
        headers = {}
        response = getHTTP(url=url, headers=headers, cookies=cookies)
        ...
        data = urlparse.urlencode({'username': username, 'password': password, 'credentialId': ''})
        headers = {
        ...
        response = postHTTP(url=url, data=data, headers=headers, cookies=cookies, allow_redirects=False)
...
def getAuthToken(auth_code: str, token: dict, brand: str) -> dict:
    ...
    try:
        ...
        data = 'client_id=' +getString("client_id", brand) +'&grant_type=authorization_code&code=' + \
            auth_code +'&redirect_uri=' + getString("base_url", brand) + '%2Fapi%2Fv1%2Fuser%2Foauth2%2Fredirect'
        headers = {
...
def registerDevice(token: dict, brand: str) -> None:
    ...
    try:
        url = getString("base_url", brand) + '/api/v1/spa/notifications/' + token["deviceId"] + '/register'
        data = {}
        headers = {
...
def refreshToken(token: dict, brand: str) -> dict:
    ...
    try: 
        url = getString("base_url", brand) + '/api/v1/user/oauth2/token'
        data = 'client_id='+ getString("client_id", brand) +'&grant_type=refresh_token&refresh_token='+ token["refreshToken"] +'&redirect_uri=' + \
            getString("base_url", brand) + '%2Fapi%2Fv1%2Fuser%2Foauth2%2Fredirect'
        headers = {
            'Authorization': getString("basic_token", brand),
            'Ccsp-Device-Id': token["deviceId"],
            'Ccsp-Service-Id': getString("client_id", brand),
        ...
        log.debug("kia.refreshToken: refresh token error: " + response, exc_info = True)
        raise

    log.debug("kia.refreshToken: New access token = " + token["accessToken"][:8] + "...")
...
def getControlToken(pin: str, token: dict, brand: str) -> str:
    ...
    try:
        ...
        log.debug("kia.getControlToken: Sending PIN error: " + response, exc_info = True)
        raise
        
    log.debug("kia.refreshToken: control token = " + control_token[7:15] + "...")
...
def getVehicleId(vin: str, token: dict, brand: str) -> str:
    ...
    log.debug("kia.getVehicleId: VehicleId = " + vehicle_id[:8] + "... (" + vehicle_name + ")")
...
def doPrewakeup(vehicle_id: str, token: dict, brand: str) -> None:
    ...
    try:
        url = getString("base_url", brand) + '/api/v1/spa/vehicles/' + vehicle_id + '/control/engine'
...
def getStatusFull(vehicle_id: str, control_token: str, token: dict, brand: str) -> CarState:
    log.info("Kia/Hyundai: Triggering Update")

    try:
        url = getString("base_url", brand) + '/api/v2/spa/vehicles/' + vehicle_id + '/ccs2/carstatus'
        headers = {
        ...
        log.debug("kia.getStatusFull: triggering update error: " + response, exc_info = True)
        raise
    ...
    try:
        url = getString("base_url", brand) + '/api/v1/spa/vehicles/' + vehicle_id + '/ccs2/carstatus/latest'
        headers = {
            'Authorization': token["tokenType"] + ' ' + token["accessToken"],
            'Ccsp-Device-Id': token["deviceId"],
            'Ccsp-Service-Id': getString("client_id", brand),
        ...
        soc = int(response_dict['resMsg']['state']['Vehicle']['Green']['BatteryManagement']['BatteryRemain']['Ratio'])
        range = float(response_dict['resMsg']['state']['Vehicle']['Drivetrain']['FuelSystem']['DTE']['Total'])
        
    except Exception as err:
        log.error("kia.getStatusFull: receiving update failed")
        log.debug("kia.getStatusFull: receiving update error: " + response, exc_info = True)
        raise
...
def fetch_soc(user_id: str, password: str, pin: str, vin: str, vehicle: int) -> CarState:
