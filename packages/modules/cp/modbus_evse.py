    ...
    try:
        ...
        if cp_num == 1:
            if cp.data["config"]["connection_module"]["modbus_evse"]["id"] == 0:
                if "source" in cp.data["config"]["connection_module"]["modbus_evse"]:
                    ...
                    if stat.S_ISBLK(os.stat("/dev/ttyUSB0").st_mode):
                        source = "/dev/ttyUSB0"
                    else:
                        source = "/dev/serial0"
                    cp.data["config"]["connection_module"]["modbus_evse"]["source"] = source
                    pub.pub("openWB/set/chargepoint/"+str(cp_num)+"/config",
                            {"connection_module": {"config": {"modbus_evse": {"source": source}}}})
                    id = 1
        else:
            source = cp.data["config"]["connection_module"]["modbus_evse"]["source"]
        ...
        if state == "" or re.search("^[0-9]+$", state) == None:
            ...
            log.message_debug_log(
                "error", "Modbus EVSE read CP"+str(cp_num)+" issue - using previous state")
        if 0 <= state <= 10:
            # LP 2 if 0 <= state <= 7:
            if state > 1:
                plug_state = True
            ...
            pub.pub("openWB/set/chargepoint/"+str(cp_num) +
                    "/get/plug_state", plug_state)
            if plug_state > 2:
                charge_state = True
            else:
                charge_state = False
            pub.pub("openWB/set/chargepoint/"+str(cp_num) +
                    "/get/charge_state", charge_state)
...
def _read_regs(source, id, reg, num):
    try:
        client = ModbusSerialClient(
            method="rtu", port=source, baudrate=9600, stopbits=1, bytesize=8, timeout=1)
        ...
        if cp.data["config"]["connection_module"]["modbus_evse"]["id"] == 0:
            if "source" in cp.data["config"]["connection_module"]["modbus_evse"]:
                ...
                if stat.S_ISBLK(os.stat("/dev/ttyUSB0").st_mode):
                    ...
                    source = "/dev/serial0"
                cp.data["config"]["connection_module"]["modbus_evse"]["source"] = source
                pub.pub("openWB/set/chargepoint/"+str(cp.cp_num)+"/config",
                        {"connection_module": {"config": {"modbus_evse": {"source": source}}}})
                id = 1
...
def check_modbus_evse(cp):
    try:
        ...
        if evsedinstat == cp.data["set"]["current"]:
            log.message_debug_log(
                "debug", "LP"+str(cp.cp_num)+" Modbus-Stromstaerke ist korrekt.")
        else:
            log.message_debug_log("error", "LP"+str(cp.cp_num) +
                                  " Modbus-Stromstaerke "+str(evsedinstat)+" ist nicht korrekt.")
            _write_regs(source, id, 1000, cp.data["set"]["current"])
    except Exception as e:
        log.exception_logging(e)
