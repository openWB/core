...
class Lovato:
    ...
    def get_imported(self) -> float:
        ...
        """
        try:
            imported = self.client.read_float_registers(
                0x0048, 2, self.id) * 1000
            return imported
        except Exception as e:
            log.MainLogger().exception("Fehler beim Auslesen von "+str(self.name))
            return None
    ...
    def get_power(self) -> Tuple[List[int], float]:
        try:
            ...
            if self.type == "Bat-Kit":
                power_all = self.client.read_float_registers(
                    0x000C, 2, self.id)
            else:
    ...
    def get_exported(self) -> float:
        ...
        """
        try:
            exported = self.client.read_float_registers(
                0x004a, 2, self.id) * 1000
            return exported
        except Exception as e:
            log.MainLogger().exception("Fehler beim Auslesen von "+str(self.name))
            return None
    ...
    def get_frequency(self) -> float:
        ...
        """
        try:
            frequency = float(self.client.read_registers(
                0x0031, 2, self.id)) / 100
            if frequency > 100:
    ...
    def get_counter(self) -> float:
        try:
            finalbezug1 = self.client.read_integer_registers(
                0x1a1f, 2, self.id)
