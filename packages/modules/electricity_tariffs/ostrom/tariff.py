...
def fetch_prices(config: OstromTariffConfiguration) -> Dict[int, float]:
    ...
    raw_prices = req.get_http_session().get(
        url = f"https://production.ostrom-api.io/spot-prices?startDate={startDate}&endDate={endDate}&resolution=HOUR&zip={config.zip}",
        headers = {
            "accept": "application/json",
    ...
    for raw_price in raw_prices:
        ...
        timestamp = datetime.strptime(raw_price["date"], "%Y-%m-%dT%H:%M:%S.000Z").replace(tzinfo=timezone.utc).timestamp()
