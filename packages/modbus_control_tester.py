...
REGISTERS = (
    ...
    Register(10160, Actions.READ_STR, 10, ModbusDataType.INT_16, name="RFID Tag", expected="any tag"),
)
    ...
    if reg.action == Actions.READ_NUMBER:
        ...
        if resp:
            for word in resp:
                if word != 0:
                    string += struct.pack(">h", word).decode("utf-8")
        # Warte auf RFID-Tag 10160
        if string == reg.expected or reg.reg == 10160:
            return string
        else:
            raise Exception(
                f"Register: {reg.reg}, Name: {reg.name} mit Ergebnis: "
