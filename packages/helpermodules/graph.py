...
def pub_graph_data():
    ...
    for c in data.data.counter_data:
        if "counter" in c and not evu_counter in c:
            ...
            dataline.update({
                "counter" + str(counter.counter_num) + "-power":
                _convert_to_kW(counter.data["get"]["power_all"])
            })
    dataline.update({
        "house-power":
        _convert_to_kW(
            data.data.counter_data["all"].data["set"]["home_consumption"])
    })
    dataline.update({
        "charging-all":
        _convert_to_kW(data.data.cp_data["all"].data["get"]["power_all"])
    })
    if len(data.data.pv_data) > 1:
        dataline.update({
            "pv-all":
            _convert_to_kW(data.data.pv_data["all"].data["get"]["power"])
        })
    if len(data.data.cp_data) > 1:
        for cp in data.data.cp_data:
            if "cp" in cp:
                ...
                dataline.update({
                    "cp" + str(chargepoint.cp_num) + "-power":
                    _convert_to_kW(chargepoint.data["get"]["power_all"])
                })
                if chargepoint.data["get"]["connected_vehicle"]["soc_config"][
                        "configured"]:
                    dataline.update({
                        "cp" + str(chargepoint.cp_num) + "-soc":
                        _convert_to_kW(chargepoint.data["get"]
                                       ["connected_vehicle"]["soc"]["soc"])
                    })
    if len(data.data.bat_data) > 1:
        dataline.update({
            "bat-all-power":
            _convert_to_kW(data.data.bat_data["all"].data["get"]["power"])
        })
        dataline.update({
            "bat-all-soc":
            _convert_to_kW(data.data.bat_data["all"].data["get"]["soc"])
        })
    # # smarthome 1
