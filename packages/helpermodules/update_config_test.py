...
@pytest.mark.parametrize("ev0_prio, ev0_chargemode, ev1_prio, ev1_chargemode, ev2_prio, ev2_chargemode, id_ordered", [
    ...
    pytest.param(False, Chargemode.INSTANT_CHARGING.value, False, Chargemode.INSTANT_CHARGING.True,
                 False, Chargemode.INSTANT_CHARGING.value, [1, 0, 2], id="Prioritäten unterschiedlich")
]
)
def test_upgrade_datastore_133_ev_chargemode_conversion(ev0_prio: bool,
                                                        ev0_chargemode: str,
                                                        ev1_prio: bool,
                                                        ev1_chargemode: str,
                                                        ev2_prio: bool,
    ...
    uc = UpdateConfig()
    uc.all_received_topics = {
        "openWB/vehicle/0/charge_template": dataclass_utils.asdict(ev0_charge_template),
        "openWB/vehicle/1/charge_template": dataclass_utils.asdict(ev1_charge_template),
        "openWB/vehicle/2/charge_template": dataclass_utils.asdict(ev2_charge_template),
        "openWB/system/datastore_version": [131, 132],
    }

    # execution
    uc.upgrade_datastore_133()
    ...
    assert uc.all_received_topics["openWB/system/datastore_version"] == [131, 132, 133]
    assert mock_pub.pub.call_count == 1  # einmal publishen für Upgrade der Datastore-Version
