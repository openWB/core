...
class SubData:
    ...
    def on_message(self, client, userdata, msg):
        ...
        if "openWB/vehicle/template/charge_template/" in msg.topic:
            ...
            log.MainLogger().warning("unknown subdata-topic: " +
                                     str(msg.topic))
    ...
    def process_vehicle_topic(self, var, msg):
        ...
        try:
            ...
            if re.search("^.+/vehicle/[0-9]+$", msg.topic) is not None:
                ...
                elif re.search("^.+/vehicle/[0-9]+/soc/get/.+$",
                               msg.topic) is not None:
                    if "soc" not in var["ev" + index].data:
                        var["ev" + index].data["soc"] = {}
                    if "get" not in var["ev" + index].data["soc"]:
                        var["ev" + index].data["soc"]["get"] = {}
                    self.set_json_payload(var["ev" + index].data["soc"]["get"],
                                          msg)
                elif re.search("^.+/vehicle/[0-9]+/control_parameter/.+$",
                               msg.topic) is not None:
    ...
    def process_vehicle_charge_template_topic(self, var, msg):
        ...
        try:
            ...
            if re.search(
                    "^.+/vehicle/template/charge_template/[0-9]+/chargemode/scheduled_charging/plans/[0-9]+$",
                    msg.topic) is not None:
                index_second = self.get_second_index(msg.topic)
                if str(msg.payload.decode("utf-8")) == "":
                    if "ct" + index in var["ct" + index].data["chargemode"][
                            "scheduled_charging"]["plans"]:
                        var.pop("ct" + index)
                    else:
                        log.MainLogger().error(
                            "Es konnte kein Zielladen-Plan mit der ID " +
                            str(index_second) + " in der Ladevorlage " +
                            str(index) + " gefunden werden.")
                else:
                    var["ct" + index].data["chargemode"]["scheduled_charging"][
                        "plans"][str(index)] = json.loads(
                            str(msg.payload.decode("utf-8")))
            elif re.search(
                    "^.+/vehicle/template/charge_template/[0-9]+/time_charging/plans/[0-9]+$",
                    msg.topic) is not None:
                index_second = self.get_second_index(msg.topic)
                if str(msg.payload.decode("utf-8")) == "":
                    if "ct" + index in var[
                            "ct" + index].data["time_charging"]["plans"]:
                        var.pop("ct" + index)
                    else:
                        log.MainLogger().error(
                            "Es konnte kein Zeitladen-Plan mit der ID " +
                            str(index_second) + " in der Ladevorlage " +
                            str(index) + " gefunden werden.")
                else:
                ...
                try:
                    scheduled_charging_plans = var["ct" + index].data[
                        "chargemode"]["scheduled_charging"]["plans"][str(
                            index)]
                except Exception:
                    pass
                try:
                    time_charging_plans = var[
                        "ct" +
                        index].data["time_charging"]["plans"][str(index)]
                except Exception:
                    pass
                var["ct" + index].data = json.loads(
                    str(msg.payload.decode("utf-8")))
                try:
                    var["ct" + index].data["time_charging"]["plans"][str(
                ...
                try:
                    var["ct" + index].data["chargemode"]["scheduled_charging"][
                        "plans"][str(index)] = scheduled_charging_plans
                except Exception:
                    pass
    ...
    def process_vehicle_ev_template_topic(self, var, msg):
        ...
        try:
            index = self.get_index(msg.topic)
            if re.search("^.+/vehicle/template/ev_template/[0-9]+$",
                         msg.topic) is not None:
                if str(msg.payload.decode("utf-8")) == "":
                    if "et" + index in var:
                        var.pop("et" + index)
        ...
        try:
            if re.search("^.+/chargepoint/[0-9]+$", msg.topic) is not None:
                ...
                if str(msg.payload.decode("utf-8")) == "":
                    if "cp" + index in var:
                        var.pop("cp" + index)
            elif re.search("^.+/chargepoint/[0-9]+/.+$",
                           msg.topic) is not None:
                index = self.get_index(msg.topic)
                ...
                if re.search("^.+/chargepoint/[0-9]+/set/.+$",
                             msg.topic) is not None:
                    if "set" not in var["cp" + index].data:
                        var["cp" + index].data["set"] = {}
                    if re.search("^.+/chargepoint/[0-9]+/set/log/.+$",
                                 msg.topic) is not None:
                        ...
                        self.set_json_payload(var["cp" + index].data["set"],
                                              msg)
                elif re.search("^.+/chargepoint/[0-9]+/get/.+$",
                               msg.topic) is not None:
                    if "get" not in var["cp" + index].data:
                        var["cp" + index].data["get"] = {}
                    if re.search(
                            "^.+/chargepoint/[0-9]+/get/connected_vehicle/.+$",
                            msg.topic) is not None:
                        if "connected_vehicle" not in var["cp" +
                                                          index].data["get"]:
                            var["cp" +
                                index].data["get"]["connected_vehicle"] = {}
                        self.set_json_payload(
                            var["cp" + index].data["get"]["connected_vehicle"],
                            msg)
                    elif re.search("^.+/chargepoint/[0-9]+/get/.+$",
                                   msg.topic) is not None:
                        self.set_json_payload(var["cp" + index].data["get"],
                                              msg)
                elif re.search("^.+/chargepoint/[0-9]+/config$",
                               msg.topic) is not None:
    ...
    def process_chargepoint_template_topic(self, var, msg):
        ...
        try:
            ...
            if json.loads(str(msg.payload.decode("utf-8"))):
                ...
                if "cpt" + index in var:
                    var.pop("cpt" + index)
            if re.search("^.+/chargepoint/template/[0-9]+/autolock/.+$",
                         msg.topic) is not None:
                if "autolock" not in var["cpt" + index].data:
                    var["cpt" + index].data["autolock"] = {}
                index_second = self.get_second_index(msg.topic)
                if "plan" + index_second not in var["cpt" +
                                                    index].data["autolock"]:
                ...
                try:
                    autolock_plans = var["cpt" +
                                         index].data["autolock"]["plans"]
                except Exception:
                    pass
                var["cpt" + index].data = json.loads(
                    str(msg.payload.decode("utf-8")))
                try:
                    var["cpt" +
                        index].data["autolock"]["plans"] = autolock_plans
                except Exception:
                    pass
        except Exception:
            log.MainLogger().exception("Fehler im subdata-Modul")
    ...
    def process_general_topic(self, var, msg):
        ...
        try:
            if re.search("^.+/general/.+$", msg.topic) is not None:
                ...
                if re.search("^.+/general/notifications/.+$",
                             msg.topic) is not None:
                    if "notifications" not in var["general"].data:
                        var["general"].data["notifications"] = {}
                    self.set_json_payload(var["general"].data["notifications"],
                                          msg)
                ...
                elif re.search("^.+/general/chargemode_config/.+$",
                               msg.topic) is not None:
                    if "chargemode_config" not in var["general"].data:
                        var["general"].data["chargemode_config"] = {}
                    if re.search(
                            "^.+/general/chargemode_config/pv_charging/.+$",
                            msg.topic) is not None:
                        if "pv_charging" not in var["general"].data[
                                "chargemode_config"]:
                            var["general"].data["chargemode_config"][
                                "pv_charging"] = {}
                        self.set_json_payload(
                            ...
                            "^.+/general/chargemode_config/instant_charging/.+$",
                            msg.topic) is not None:
                        if "instant_charging" not in var["general"].data[
                                "chargemode_config"]:
                            var["general"].data["chargemode_config"][
                                "instant_charging"] = {}
                        self.set_json_payload(
                            var["general"].data["chargemode_config"]
                            ["instant_charging"], msg)
                    elif re.search(
                            "^.+/general/chargemode_config/scheduled_charging/.+$",
                            msg.topic) is not None:
                        if "scheduled_charging" not in var["general"].data[
                                "chargemode_config"]:
                            var["general"].data["chargemode_config"][
                                "scheduled_charging"] = {}
                        self.set_json_payload(
                            var["general"].data["chargemode_config"]
                            ["scheduled_charging"], msg)
                    elif re.search(
                            "^.+/general/chargemode_config/time_charging/.+$",
                            msg.topic) is not None:
                        if "time_charging" not in var["general"].data[
                                "chargemode_config"]:
                            var["general"].data["chargemode_config"][
                                "time_charging"] = {}
                        self.set_json_payload(
                            var["general"].data["chargemode_config"]
                            ["time_charging"], msg)
                    elif re.search("^.+/general/chargemode_config/standby/.+$",
                                   msg.topic) is not None:
                        if "standby" not in var["general"].data[
                                "chargemode_config"]:
                            var["general"].data["chargemode_config"][
                                "standby"] = {}
                        self.set_json_payload(
    ...
    def process_counter_topic(self, var, msg):
        ...
        try:
            if re.search("^.+/counter/[0-9]+$", msg.topic) is not None:
                ...
                if re.search("^.+/counter/[0-9]+/get.+$",
                             msg.topic) is not None:
                    if "get" not in var["counter" + index].data:
                        var["counter" + index].data["get"] = {}
                    self.set_json_payload(var["counter" + index].data["get"],
                                          msg)
                elif re.search("^.+/counter/[0-9]+/set.+$",
                               msg.topic) is not None:
                    if "set" not in var["counter" + index].data:
                        var["counter" + index].data["set"] = {}
                    self.set_json_payload(var["counter" + index].data["set"],
                                          msg)
                elif re.search("^.+/counter/[0-9]+/config/.+$",
                               msg.topic) is not None:
    ...
    def process_system_topic(self, client, var, msg):
        ...
        try:
            ...
            if re.search("^.+/device/[0-9]+/config$", msg.topic) is not None:
                ...
                if str(msg.payload.decode("utf-8")) == "":
                    if "device" + index in var:
                        ...
                        log.MainLogger().error(
                            "Es konnte kein Device mit der ID " + str(index) +
                            " gefunden werden.")
                else:
                    device_config = json.loads(str(
                        msg.payload.decode("utf-8")))
                    dev = importlib.import_module(
                        ".modules." + device_config["type"] + ".device",
                        "packages")
                    ...
                    client.subscribe(
                        "openWB/system/device/" + index + "/component/#", 2)
            ...
            elif re.search("^.+/device/[0-9]+/component/[0-9]+/simulation/.+$",
                           msg.topic) is not None:
                index = self.get_index(msg.topic)
                index_second = self.get_second_index(msg.topic)
                self.set_json_payload(
                    var["device" + index].data["components"][
                        "component" + index_second].data["simulation"], msg)
            elif re.search("^.+/device/[0-9]+/component/[0-9]+/config$",
                           msg.topic) is not None:
                index = self.get_index(msg.topic)
                index_second = self.get_second_index(msg.topic)
                if str(msg.payload.decode("utf-8")) == "":
                    if "device" + index in var:
                        if "component" + str(index_second) in var[
                                "device" + index].data["components"]:
                            var["device" + index].data["components"].remove(
                                "component" + str(index_second))
                            pub.pub(
                                "openWB/system/device/" + str(index) +
                                "/component/" + str(index_second), "")
                        else:
                            log.MainLogger().error(
                                "Es konnte keine Komponente mit der ID " +
                    ...
                    sim_data = None
                    if "component" + index_second in var[
                            "device" + index].data["components"]:
                        sim_data = var["device" + index].data["components"][
                            "component" + index_second].data["simulation"]
