...
class Command:
    ...
    def __init__(self):
        try:
            self.__get_max_id("autolock_plan",
                              "chargepoint/template/+/autolock")
                ...
                if "todo" in msg.topic:
                    ...
                    try:
                        ...
                        log.MainLogger().error(
                            "Zu dem Befehl wurde keine Methode gefunden.")
                        self.__pub_error(
    ...
    def __pub_error(self, payload: dict, connection_id: str,
                    error_str: str) -> None:
        ...
        try:
            ...
            pub.pub("openWB/set/command/" + str(connection_id) + "/error",
                    error_payload)
            log.MainLogger().error("Befehl konnte nicht ausgefuehrt werden: " +
                                   str(error_payload))
        except Exception as e:
            log.MainLogger().exception("Fehler im Command-Modul")
    ...
    def addDevice(self, connection_id: str, payload: dict) -> None:
        ...
        try:
            ...
            log.MainLogger().info("Neues Device vom Typ" +
                                  str(payload["data"]["type"]) + " mit ID " +
                                  str(new_id) + " hinzugefuegt.")
            ...
            pub.pub("openWB/set/system/device/" + str(new_id) + "/config",
                    device_default)
    ...
    def removeDevice(self, connection_id: str, payload: dict) -> None:
        ...
        try:
            if self.max_id_device >= payload["data"]["id"]:
                ...
                self.__pub_error(
                    payload, connection_id,
                    "Die ID ist groesser als die maximal vergebene ID.")
        except Exception as e:
            log.MainLogger().exception("Fehler im Command-Modul")
            self.__pub_error(
                payload, connection_id,
    ...
    def addChargepoint(self, connection_id: str, payload: dict) -> None:
        ...
        try:
            ...
            data.data.counter_data["all"].hierarchy_add_item_below(
                "cp" + str(new_id), data.data.counter_data["all"].data["get"]
                ["hierarchy"][0]["id"])
            pub.pub("openWB/set/chargepoint/" + str(new_id) + "/config",
                    chargepoint_default)
    ...
    def removeChargepointTemplate(self, connection_id: str,
                                  payload: dict) -> None:
    ...
    def addAutolockPlan(self, connection_id: str, payload: dict) -> None:
        ...
        try:
            ...
            log.MainLogger().info("Neuer Autolock-Plan mit ID " + str(new_id) +
                                  " zu Template " +
                                  str(payload["data"]["template"]) +
                                  " hinzugefuegt.")
            default = chargepoint.get_autolock_plan_default()
            pub.pub(
                "openWB/set/chargepoint/template/" +
                str(payload["data"]["template"]) + "/autolock/" + str(new_id),
                default)
        ...
        try:
            if self.max_id_autolock_plan >= payload["data"]["plan"]:
                ...
                pub.pub(
                    ...
                    str(payload["data"]["template"]) + "/autolock/" +
                    str(payload["data"]["plan"]), "")
            else:
                self.__pub_error(
                    payload, connection_id,
    ...
    def addChargeTemplate(self, connection_id: str, payload: dict) -> None:
        ...
        try:
            ...
            log.MainLogger().info("Neues Lade-Template mit ID " + str(new_id) +
                                  " hinzugefuegt.")
            charge_template_default = ev.get_charge_template_default()
            pub.pub(
                "openWB/set/vehicle/template/charge_template/" + str(new_id),
    ...
    def addChargeTemplateSchedulePlan(self, connection_id: str,
                                      payload: dict) -> None:
        """ sendet das Topic, zu dem ein neuer Zielladen-Plan erstellt werden soll.
        """
        try:
            new_id = self.max_id_charge_template_scheduled_plan + 1
            log.MainLogger().info("Neues Zielladen-Template mit ID " +
                                  str(new_id) + " zu Template " +
                                  str(payload["data"]["template"]) +
                                  " hinzugefuegt.")
            charge_template_default = ev.get_charge_template_scheduled_plan_default(
            )
            pub.pub(
                "openWB/set/vehicle/template/charge_template/" +
                str(payload["data"]["template"]) +
    ...
    def removeChargeTemplateSchedulePlan(self, connection_id: str,
                                         payload: dict) -> None:
        """ löscht einen Zielladen-Plan.
        """
        try:
            if self.max_id_charge_template_scheduled_plan >= payload["data"][
                    "plan"]:
    ...
    def addChargeTemplateTimeChargingPlan(self, connection_id: str,
                                          payload: dict) -> None:
        ...
        try:
            ...
            log.MainLogger().info("Neues Zeitladen-Template mit ID " +
                                  str(new_id) + " zu Template " +
                                  str(payload["data"]["template"]) +
                                  " hinzugefuegt.")
            ...
            pub.pub(
                "openWB/set/vehicle/template/charge_template/" +
                str(payload["data"]["template"]) + "/time_charging/plans/" +
                str(new_id), time_charging_plan_default)
            self.max_id_charge_template_time_charging_plan = new_id
            pub.pub(
                "openWB/set/command/max_id/charge_template_time_charging_plan",
                new_id)
        except Exception as e:
            log.MainLogger().exception("Fehler im Command-Modul")
            self.__pub_error(
    ...
    def removeChargeTemplateTimeChargingPlan(self, connection_id: str,
                                             payload: dict) -> None:
        """ löscht einen Zeitladen-Plan.
        """
        try:
            if self.max_id_charge_template_time_charging_plan >= payload[
                    "data"]["plan"]:
    ...
    def addComponent(self, connection_id: str, payload: dict) -> None:
        ...
        try:
            ...
            component = importlib.import_module(
                ".modules." + payload["data"]["deviceType"] + "." +
                payload["data"]["type"], "packages")
            ...
            if payload["data"]["type"] == "counter":
                data.data.counter_data["all"].hierarchy_add_item_below(
                    ...
                    ["id"])
            pub.pub(
                "openWB/set/system/device/" +
                str(payload["data"]["deviceId"]) + "/component/" +
                str(new_id) + "/config", component_default)
        ...
        try:
            if self.max_id_component >= payload["data"]["id"]:
                ...
                log.MainLogger().info("Komponente mit ID " +
                                      ...
                                      " geloescht.")
                ProcessBrokerBranch(
                    "system/device/" + str(payload["data"]["deviceId"]) +
                    "/component/" +
                    str(payload["data"]["id"])).remove_topics()
            else:
                self.__pub_error(
    ...
    def addEvTemplate(self, connection_id: str, payload: dict) -> None:
        ...
        try:
            ...
            log.MainLogger().info("Neues EV-Template mit ID " + str(new_id) +
                                  " hinzugefuegt.")
            ev_template_default = ev.get_ev_template_default()
            pub.pub("openWB/set/vehicle/template/ev_template/" + str(new_id),
                    ev_template_default)
    ...
    def addVehicle(self, connection_id: str, payload: dict) -> None:
        ...
        try:
            ...
            log.MainLogger().info("Neues EV mit ID " + str(new_id) +
                                  " hinzugefuegt.")
            vehicle_default = ev.get_vehicle_default()
            for default in vehicle_default:
                pub.pub(
                    "openWB/set/vehicle/" + str(new_id) + "/" + str(default),
...
class ProcessBrokerBranch:
    ...
    def __connect_to_broker(self, on_message):
        ...
        try:
            mqtt_broker_ip = "localhost"
            client = mqtt.Client("openWB-proccesBrokerBranch-" +
                                 str(self.__getserial()))
    ...
    def __on_message_max_id(self, client, userdata, msg):
        try:
            topic_found = re.search('^(' + self.search_str + '/*).*$',
                                    msg.topic).group(1)
