        ...
        try:
            ...
            if os.path.isfile('/var/www/html/openWB/ramdisk/'+prefix+'sec0'):
                ...
                log.MainLogger().debug("simcount Zwischenergebnisse letzte Berechnung: Import: " +
                                       str(counter_import_previous)+" Export: "+str(counter_export_previous)+" Power: "+str(power_previous))
                start_new = False
            ...
            if start_new is False:
                ...
                log.MainLogger().debug("simcount aufsummierte Energie: Bezug[Ws]: "+str(
                    counter_import_present)+", Einspeisung[Ws]: "+str(counter_export_present))
                ...
                log.MainLogger().debug("simcount Zwischenergebnisse atkuelle Berechnung: Import: " +
                                       str(counter_import_present)+" Export: "+str(counter_export_present)+" Power: "+str(power_present))
                self.write_ramdisk_file(
                    prefix+'watt0pos', counter_import_present)
    ...
    def sim_count(self, power_present: float, topic: str = "", data: dict = {}, prefix: str = "") -> typing.Tuple[float, float]:
        ...
        try:
            ...
            if "timestamp_present" in data:
                ...
                log.MainLogger().debug("Fortsetzen der Simulation: Importzaehler: " +
                                       str(counter_import_present)+"Ws, Export-Zaehler: "+str(counter_export_present)+"Ws")
                start_new = False
            ...
            if start_new is False:
                ...
                log.MainLogger().debug("simcount Zwischenergebnisse atkuelle Berechnung: Import: " +
                                       str(counter_import_present)+" Export: "+str(counter_export_present)+" Power: "+str(power_present))
                pub.pub(topic+"simulation/present_imported",
                        counter_import_present)
                pub.pub(topic+"simulation/present_exported",
...
def calculate_import_export(seconds_since_previous: Number, power1: Number, power2: Number) -> typing.Tuple[Number, Number]:
    try:
        log.MainLogger().debug("simcount Berechnungsgrundlage: vergangene Zeit [s]"+str(
            seconds_since_previous)+", vorherige Leistung[W]: "+str(power1)+", aktuelle Leistung[W]: "+str(power2))
        ...
        def energy_function(seconds): return .5 * gradient * \
            seconds ** 2 + power_low * seconds
