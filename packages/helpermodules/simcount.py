    ...
    def get_sim_counter(self):
        try:
            ramdisk = Path(
                ...
                "/ramdisk/bootinprogress").is_file()
        ...
        try:
            ...
            if os.path.isfile('/var/www/html/openWB/ramdisk/' + prefix +
                ...
                log.MainLogger().debug(
                    ...
                    str(power_previous))
                start_new = False
            self.write_ramdisk_file(prefix + 'sec0',
        ...
        try:
            if prefix == "bezug":
                ...
                log.MainLogger().error(
                    "Fehler im Modul simcount: Unbekannter Präfix")
            return topic
    ...
    def restore(self, value, prefix: str):
        ...
        try:
            ...
            try:
                topic = self.__get_topic(prefix)
                if value == "watt0pos":
                    temp = subscribe.simple("openWB/" + topic +
                                            "/WHImported_temp",
                                            hostname="localhost")
                else:
                    temp = subscribe.simple("openWB/" + topic +
            ...
            if value == "watt0pos":
                log.MainLogger().info("loadvars read openWB/" + topic +
                                      "/WHImported_temp from mosquito " +
                                      str(temp))
            else:
                log.MainLogger().info("loadvars read openWB/" + topic +
                                      "/WHExport_temp from mosquito " +
                                      str(temp))
            return temp
        except Exception:
            log.MainLogger().exception("Fehler im Modul simcount")
    ...
    def sim_count(self,
        ...
        try:
            ...
            if "timestamp_present" in data:
                ...
                log.MainLogger().debug(
                    "Fortsetzen der Simulation: Importzaehler: " +
                    str(counter_import_present) + "Ws, Export-Zaehler: " +
