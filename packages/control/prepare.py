...
from control.chargepoint import ConnectedConfig, ConnectedInfo, ConnectedSoc
...
class Prepare:
    ...
    def _pub_connected_vehicle(self, vehicle, chargepoint):
        ...
        try:
            ...
            if soc_config_obj != chargepoint.data.get.connected_vehicle.soc:
            ...
            if soc_obj != chargepoint.data.get.connected_vehicle.soc:
                Pub().pub("openWB/chargepoint/"+str(chargepoint.num) +
                          "/get/connected_vehicle/soc", asdict(soc_obj))
            if info_obj != chargepoint.data.get.connected_vehicle.info:
                Pub().pub("openWB/chargepoint/"+str(chargepoint.num) +
                          "/get/connected_vehicle/info", asdict(info_obj))
            if config_obj != chargepoint.data.get.connected_vehicle.config:
                Pub().pub("openWB/chargepoint/"+str(chargepoint.num) +
                          "/get/connected_vehicle/config", asdict(config_obj))
        except Exception:
            log.exception("Fehler im Prepare-Modul")
