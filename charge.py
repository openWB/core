...
class charge():
                ...
                if "cp" in cp:
                    ...
                    if "set" in chargepoint.data:
                        if "charging_ev" in chargepoint.data["set"]:
                            # Hat sich die Stromstärke oder Anzahl der Phasen geändert?
                            if (chargepoint.data["get"]["phases_in_use"] != chargepoint.data["set"]["phases_to_use"] or 
                                    chargepoint.data["set"]["current"] != chargepoint.data["get"]["current"]):
    ...
    def _update_state(self, chargepoint):
        ...
        try:
            ...
            pub.pub("openWB/set/chargepoint/"+str(chargepoint.cp_num)+"/set/current", chargepoint.data["set"]["current"])
